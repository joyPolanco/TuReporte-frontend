...
import {
  ...
  CheckSquare, Square, PlayCircle, Ban, ClipboardCheck, Plus, Wrench, CalendarClock, History, Settings2,
  Flame,
} from "lucide-react";
import {
...
const PERMISSIONS = {
  ciudadano:  { crear: true, aportarInfo: true, reabrirPropio: true, calificar: true, confirmarResolucion: true },
  gestor:     { revisar: true, validar: true, clasificar: true, asignar: true, reasignar: true, escalar: true, marcarParaVerificacion: true, cerrar: true, reabrir: true, verificar: true, coordinarInstituciones: true, administrarCatalogo: false, administrarUsuarios: false, administrarInstituciones: false, verAuditoria: true },
  supervisor: { revisar: true, validar: true, clasificar: true, asignar: true, reasignar: true, escalar: true, marcarParaVerificacion: true, cerrar: true, reabrir: true, verificar: true, resolverEscalamientos: true, coordinarInstituciones: true, administrarCatalogo: false, administrarUsuarios: false, administrarInstituciones: false, verAuditoria: true },
  admin:      { revisar: true, validar: true, clasificar: true, asignar: true, reasignar: true, escalar: true, marcarParaVerificacion: true, cerrar: true, reabrir: true, verificar: true, resolverEscalamientos: true, coordinarInstituciones: true, administrarCatalogo: true, administrarUsuarios: true, administrarInstituciones: true, verAuditoria: true },
};
...
const CATEGORIES = [
  { id: "calles", label: "Infraestructura vial", Icon: Construction, badge: "bg-orange-100 text-orange-700", dot: "bg-orange-500", color: "#ea580c", defaultInstitutionKind: "ayuntamiento" },
  { id: "alumbrado", label: "Alumbrado público", Icon: Lightbulb, badge: "bg-yellow-100 text-yellow-800", dot: "bg-yellow-500", color: "#ca8a04", defaultInstitutionKind: "distribuidora" },
  { id: "basura", label: "Residuos y limpieza", Icon: Trash2, badge: "bg-amber-100 text-amber-800", dot: "bg-amber-600", color: "#b45309", defaultInstitutionKind: "ayuntamiento" },
  { id: "alcantarillado", label: "Alcantarillado y saneamiento", Icon: Droplet, badge: "bg-cyan-100 text-cyan-700", dot: "bg-cyan-600", color: "#0e7490", defaultInstitutionKind: "institucion_nacional" },
  { id: "agua", label: "Suministro de agua potable", Icon: Droplets, badge: "bg-blue-100 text-blue-700", dot: "bg-blue-500", color: "#2563eb", defaultInstitutionKind: "institucion_nacional" },
  { id: "ambiente", label: "Medio ambiente", Icon: Leaf, badge: "bg-emerald-100 text-emerald-700", dot: "bg-emerald-600", color: "#059669", defaultInstitutionKind: "institucion_nacional" },
  { id: "ocupacion", label: "Espacio público", Icon: AlertTriangle, badge: "bg-purple-100 text-purple-700", dot: "bg-purple-500", color: "#7c3aed", defaultInstitutionKind: "ayuntamiento" },
  { id: "mobiliario", label: "Mobiliario urbano", Icon: Building2, badge: "bg-rose-100 text-rose-700", dot: "bg-rose-500", color: "#e11d48", defaultInstitutionKind: "ayuntamiento" },
  { id: "otro", label: "Otro", Icon: HelpCircle, badge: "bg-slate-100 text-slate-700", dot: "bg-slate-500", color: "#475569", defaultInstitutionKind: "ayuntamiento" },
];
const getCategory = (id) => CATEGORIES.find((c) => c.id === id) || CATEGORIES[CATEGORIES.length - 1];

const SUBCATEGORIES = [
  { id: "sc-bache", categoryId: "calles", label: "Bache", defaultPriority: "alta" },
  { id: "sc-semaforo", categoryId: "calles", label: "Semáforo dañado", defaultPriority: "alta" },
  { id: "sc-senalizacion", categoryId: "calles", label: "Señalización vial ausente/dañada", defaultPriority: "media" },
  { id: "sc-inundacion", categoryId: "calles", label: "Vía inundada", defaultPriority: "alta" },
  { id: "sc-via-obstruida", categoryId: "calles", label: "Vía obstruida", defaultPriority: "media" },

  { id: "sc-luminaria-apagada", categoryId: "alumbrado", label: "Luminaria apagada", defaultPriority: "media" },
  { id: "sc-poste-danado", categoryId: "alumbrado", label: "Poste dañado o inclinado", defaultPriority: "alta" },
  { id: "sc-cableado", categoryId: "alumbrado", label: "Cableado expuesto", defaultPriority: "critica" },

  { id: "sc-acumulacion", categoryId: "basura", label: "Acumulación de basura", defaultPriority: "media" },
  { id: "sc-contenedor", categoryId: "basura", label: "Contenedor desbordado", defaultPriority: "media" },
  { id: "sc-quema", categoryId: "basura", label: "Quema de basura a cielo abierto", defaultPriority: "alta" },

  { id: "sc-alcantarilla-obstruida", categoryId: "alcantarillado", label: "Alcantarilla obstruida", defaultPriority: "alta" },
  { id: "sc-tapa-faltante", categoryId: "alcantarillado", label: "Tapa de alcantarilla faltante", defaultPriority: "critica" },
  { id: "sc-aguas-residuales", categoryId: "alcantarillado", label: "Fuga de aguas residuales", defaultPriority: "alta" },

  { id: "sc-falta-suministro", categoryId: "agua", label: "Falta de suministro", defaultPriority: "alta" },
  { id: "sc-fuga-potable", categoryId: "agua", label: "Fuga de agua potable", defaultPriority: "alta" },
  { id: "sc-baja-presion", categoryId: "agua", label: "Baja presión", defaultPriority: "media" },

  { id: "sc-canada", categoryId: "ambiente", label: "Contaminación de cañada/cuerpo de agua", defaultPriority: "alta" },
  { id: "sc-arbol-caido", categoryId: "ambiente", label: "Árbol caído", defaultPriority: "alta" },
  { id: "sc-tala", categoryId: "ambiente", label: "Tala ilegal", defaultPriority: "media" },

  { id: "sc-ocupacion-acera", categoryId: "ocupacion", label: "Ocupación de acera", defaultPriority: "baja" },
  { id: "sc-venta-ambulante", categoryId: "ocupacion", label: "Venta ambulante no autorizada", defaultPriority: "baja" },
  { id: "sc-escombros", categoryId: "ocupacion", label: "Escombros en vía pública", defaultPriority: "media" },

  { id: "sc-banca-danada", categoryId: "mobiliario", label: "Banca deteriorada", defaultPriority: "baja" },
  { id: "sc-parque-danado", categoryId: "mobiliario", label: "Equipo de parque dañado", defaultPriority: "media" },

  { id: "sc-otro", categoryId: "otro", label: "Otro / no clasificado", defaultPriority: "media" },
];
const getSubcategoriesFor = (categoryId) => SUBCATEGORIES.filter((s) => s.categoryId === categoryId);
...
const INSTITUTIONS = [
  ...
  { id: "caasd", name: "CAASD — Acueducto y Alcantarillado de Santo Domingo", short: "CAASD", kind: "institucion_nacional" },
...
const DEPARTMENTS_BY_KIND = {
  ayuntamiento: ["Obras Públicas Municipales", "Ornato y Limpieza", "Ingeniería Vial", "Medio Ambiente Municipal", "Atención Ciudadana"],
  institucion_nacional: ["Operaciones", "Mantenimiento de Redes", "Ingeniería", "Atención al Usuario"],
  distribuidora: ["Mantenimiento de Redes Eléctricas", "Cuadrillas de Campo", "Atención al Cliente"],
};
const getDepartmentsFor = (institutionId) => DEPARTMENTS_BY_KIND[getInstitution(institutionId).kind] || ["Operaciones"];
...
const RESPONSABLE_NAMES = ["Juan Pérez", "María González", "Luis Almonte", "Rosa Herrera", "Pedro Castillo", "Ana Beltré", "Carlos Núñez", "Yohanny Vargas"];
function getResponsablesFor(institutionId) {
  ...
  return depts.map((dept, i) => ({
    ...
    name: RESPONSABLE_NAMES[(institutionId.length + i) % RESPONSABLE_NAMES.length],
    department: dept,
    institutionId,
  }));
}

const ESCALATION_TARGETS = ["Supervisor institucional", "Dirección General de la institución", "Institución de mayor jerarquía / MOPC"];
...
const STATUS_META = {
  nuevo:                  { label: "Nuevo",                     Icon: Sparkles,      badge: "bg-slate-100 text-slate-700 border border-slate-200",  color: "#64748b", group: "nuevas", citizenStep: 0 },
  en_revision:            { label: "En revisión",                Icon: Search,        badge: "bg-amber-100 text-amber-700 border border-amber-200",  color: "#d97706", group: "validacion", citizenStep: 1 },
  requiere_info:          { label: "Requiere información",       Icon: HelpCircle,    badge: "bg-orange-100 text-orange-700 border border-orange-200", color: "#ea580c", group: "validacion", citizenStep: 1 },
  validado:               { label: "Validado",                   Icon: BadgeCheck,    badge: "bg-teal-100 text-teal-700 border border-teal-200",     color: "#0d9488", group: "asignacion", citizenStep: 2 },
  clasificado:            { label: "Clasificado",                Icon: ListChecks,    badge: "bg-cyan-100 text-cyan-700 border border-cyan-200",     color: "#0891b2", group: "asignacion", citizenStep: 2 },
  asignado:               { label: "Asignado",                   Icon: UserPlus,      badge: "bg-blue-100 text-blue-700 border border-blue-200",     color: "#2563eb", group: "asignacion", citizenStep: 2 },
  en_gestion:             { label: "En gestión",                 Icon: RefreshCw,     badge: "bg-indigo-100 text-indigo-700 border border-indigo-200", color: "#4f46e5", group: "gestion", citizenStep: 3 },
  en_espera:              { label: "En espera",                  Icon: TimerReset,    badge: "bg-slate-200 text-slate-600 border border-slate-300",  color: "#94a3b8", group: "gestion", citizenStep: 3 },
  escalado:               { label: "Escalado",                   Icon: TrendingUp,    badge: "bg-fuchsia-100 text-fuchsia-700 border border-fuchsia-200", color: "#c026d3", group: "gestion", citizenStep: 3 },
  pendiente_verificacion: { label: "Pendiente de verificación",  Icon: Eye,           badge: "bg-purple-100 text-purple-700 border border-purple-200", color: "#9333ea", group: "verificacion", citizenStep: 4 },
  verificado:             { label: "Verificado",                 Icon: ShieldCheck,   badge: "bg-emerald-100 text-emerald-700 border border-emerald-200", color: "#059669", group: "verificacion", citizenStep: 4 },
  resuelto:               { label: "Resuelto",                   Icon: CheckCircle2,  badge: "bg-green-100 text-green-700 border border-green-200",  color: "#16a34a", group: "cerradas", citizenStep: 5 },
  cerrado:                { label: "Cerrado",                    Icon: Flag,          badge: "bg-slate-100 text-slate-600 border border-slate-200",  color: "#475569", group: "cerradas", citizenStep: 5 },
  rechazado:              { label: "Rechazado",                  Icon: X,             badge: "bg-red-100 text-red-700 border border-red-200",        color: "#dc2626", group: "rechazadas", citizenStep: -1 },
  duplicado:              { label: "Duplicado",                  Icon: Copy,          badge: "bg-rose-100 text-rose-700 border border-rose-200",     color: "#e11d48", group: "rechazadas", citizenStep: -1 },
};
const STATUS_ORDER = ["nuevo", "en_revision", "requiere_info", "validado", "clasificado", "asignado", "en_gestion", "en_espera", "escalado", "pendiente_verificacion", "verificado", "resuelto", "cerrado", "rechazado", "duplicado"];
...
const PRIORITY_META = {
  baja: { label: "Baja", badge: "bg-slate-100 text-slate-600", color: "#64748b", slaHours: 168 },
  media: { label: "Media", badge: "bg-amber-100 text-amber-700", color: "#d97706", slaHours: 72 },
  alta: { label: "Alta", badge: "bg-orange-100 text-orange-700", color: "#ea580c", slaHours: 24 },
  critica: { label: "Crítica", badge: "bg-red-100 text-red-700", color: "#dc2626", slaHours: 4 },
};
...
const DEFAULT_RULES = { routing: [], sla: [], notifySlaBreach: true, recurringMinCount: 3, recurringWindowDays: 30 };

function applyRoutingRule(rules, categoryId, subcategoryId, fallbackInstitutionId) {
  const match = (rules?.routing || []).find((r) => r.enabled && r.categoryId === categoryId && (!r.subcategoryId || r.subcategoryId === subcategoryId));
  return match ? match.institutionId : fallbackInstitutionId;
}
function applySlaHours(rules, priority, fallbackHours) {
  const match = (rules?.sla || []).find((r) => r.enabled && r.priority === priority);
...
function getRecurringThreshold(rules) {
  return { minCount: rules?.recurringMinCount || 3, windowDays: rules?.recurringWindowDays || 30 };
}
...
const TASK_STATUS_META = {
  pendiente:   { label: "Pendiente",    Icon: Square,      color: "#94a3b8", badge: "bg-slate-100 text-slate-600 border border-slate-200" },
  en_progreso: { label: "En progreso",  Icon: PlayCircle,  color: "#2563eb", badge: "bg-blue-100 text-blue-700 border border-blue-200" },
  completada:  { label: "Completada",   Icon: CheckSquare, color: "#16a34a", badge: "bg-emerald-100 text-emerald-700 border border-emerald-200" },
  bloqueada:   { label: "Bloqueada",    Icon: Ban,         color: "#dc2626", badge: "bg-red-100 text-red-700 border border-red-200" },
};
...
const DEFAULT_TASK_TEMPLATE = [
  { key: "inspeccionar", title: "Inspeccionar ubicación", description: "Confirmar en sitio la existencia y alcance del problema reportado.", dependsOnKey: null },
  { key: "causa", title: "Determinar causa", description: "Identificar el origen del problema para definir la intervención correcta.", dependsOnKey: null },
  { key: "reparar", title: "Realizar reparación", description: "Ejecutar la intervención correctiva en sitio.", dependsOnKey: "causa" },
  { key: "evidencia", title: "Agregar evidencia", description: "Documentar el trabajo realizado con fotografías.", dependsOnKey: "reparar" },
  { key: "verificar", title: "Verificar solución", description: "Confirmar que el problema quedó resuelto antes de cerrar el expediente.", dependsOnKey: "reparar" },
];

/* ------------------------- INSPECCIONES (§18 del brief) -------------------- */
const INSPECTION_RESULT_META = {
  problema_confirmado:      { label: "Problema confirmado",      color: "#dc2626", badge: "bg-red-100 text-red-700 border border-red-200" },
  problema_no_encontrado:   { label: "Problema no encontrado",   color: "#64748b", badge: "bg-slate-100 text-slate-600 border border-slate-200" },
  requiere_intervencion:    { label: "Requiere intervención",    color: "#ea580c", badge: "bg-orange-100 text-orange-700 border border-orange-200" },
  riesgo_confirmado:        { label: "Riesgo confirmado",        color: "#b91c1c", badge: "bg-red-100 text-red-800 border border-red-300" },
  informacion_insuficiente: { label: "Información insuficiente", color: "#d97706", badge: "bg-amber-100 text-amber-700 border border-amber-200" },
};
...
const TRANSITIONS = {
  nuevo: [
    { to: "en_revision", label: "Iniciar revisión", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: Search },
  ],
  en_revision: [
    { to: "validado", label: "Validar", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: BadgeCheck, tone: "emerald" },
    { to: "requiere_info", label: "Solicitar información", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: HelpCircle, tone: "amber" },
    { to: "rechazado", label: "Rechazar", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: X, tone: "red" },
    { to: "duplicado", label: "Marcar como duplicado", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: Copy, tone: "rose" },
  ],
  requiere_info: [
    { to: "en_revision", label: "Aportar información (ciudadano)", roles: ["ciudadano"], reason: true, reasonLabel: "Información adicional", Icon: Send, tone: "blue" },
    { to: "rechazado", label: "Rechazar (sin respuesta)", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: X, tone: "red" },
  ],
  validado: [
    { to: "clasificado", label: "Clasificar", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: ListChecks, tone: "cyan", needsClassification: true },
  ],
  clasificado: [
    { to: "asignado", label: "Asignar institución / responsable", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: UserPlus, tone: "blue", needsAssignment: true },
  ],
  asignado: [
    { to: "en_gestion", label: "Iniciar gestión", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: RefreshCw, tone: "indigo" },
    { to: "escalado", label: "Escalar", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: TrendingUp, tone: "fuchsia", needsEscalation: true },
  ],
  en_gestion: [
    { to: "en_espera", label: "Marcar en espera", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: TimerReset, tone: "slate" },
    { to: "escalado", label: "Escalar", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: TrendingUp, tone: "fuchsia", needsEscalation: true },
    { to: "pendiente_verificacion", label: "Marcar listo para verificación", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: Eye, tone: "purple" },
  ],
  en_espera: [
    { to: "en_gestion", label: "Reanudar gestión", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: RefreshCw, tone: "indigo" },
    { to: "escalado", label: "Escalar", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: TrendingUp, tone: "fuchsia", needsEscalation: true },
  ],
  escalado: [
    { to: "en_gestion", label: "Retomar gestión", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: RefreshCw, tone: "indigo" },
    { to: "asignado", label: "Reasignar", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: UserPlus, tone: "blue", needsAssignment: true },
  ],
  pendiente_verificacion: [
    { to: "verificado", label: "Verificar y aprobar", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: ShieldCheck, tone: "emerald" },
    { to: "en_gestion", label: "Objetar verificación (reabrir gestión)", roles: ["gestor", "supervisor", "admin"], reason: true, Icon: RefreshCw, tone: "amber" },
  ],
  verificado: [
    { to: "resuelto", label: "Marcar como resuelto", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: CheckCircle2, tone: "green" },
  ],
  resuelto: [
    { to: "cerrado", label: "Cerrar caso", roles: ["gestor", "supervisor", "admin"], reason: false, Icon: Flag, tone: "slate" },
  ],
  cerrado: [
    { to: "en_gestion", label: "Reabrir incidencia", roles: ["ciudadano", "gestor", "supervisor", "admin"], reason: true, Icon: RefreshCw, tone: "amber" },
  ],
  rechazado: [
    { to: "en_revision", label: "Reconsiderar rechazo", roles: ["admin"], reason: true, Icon: RefreshCw, tone: "amber" },
  ],
  duplicado: [
    { to: "en_revision", label: "Revertir marca de duplicado", roles: ["admin"], reason: true, Icon: RefreshCw, tone: "amber" },
  ],
};
const availableActions = (report, role) => (TRANSITIONS[report.status] || []).filter((a) => a.roles.includes(role));
...
function findRecurringProblems(report, allReports, rules) {
  ...
  const now = Date.now();
  const related = allReports.filter((r) =>
    r.category === report.category &&
    r.zone === report.zone &&
...
function findHotspots(allReports, { minCount = HOTSPOT_MIN_ACTIVE, windowDays = HOTSPOT_WINDOW_DAYS } = {}) {
  const WINDOW = windowDays * 86400000;
  const now = Date.now();
  const recentActive = allReports.filter((r) => {
    const group = STATUS_META[r.status]?.group;
    return group !== "cerradas" && group !== "rechazadas" && now - r.createdAt < WINDOW;
  });
  return ZONES.map((zone) => {
    const inZone = recentActive.filter((r) => r.zone === zone.id);
    if (inZone.length < minCount) return null;
    const byCat = {};
    inZone.forEach((r) => { byCat[r.category] = (byCat[r.category] || 0) + 1; });
    const topCat = Object.entries(byCat).sort((a, b) => b[1] - a[1])[0];
    const critical = inZone.filter((r) => r.priority === "critica" || r.priority === "alta").length;
    return { zone, count: inZone.length, topCat: { id: topCat[0], count: topCat[1] }, critical, reports: inZone };
...
const MOCK_USERS = [
  ...
  { id: "s1", name: "María González", cedula: "402-5551234-7", phone: "809-555-0311", email: "maria.gonzalez@adn.gob.do", role: "supervisor", institutionId: "ayto-dn", seed: "MariaG" },
...
const REPORT_SEEDS = [
  { desc: "Bache profundo en la Av. Independencia que ya ha dañado varios vehículos.", category: "calles", subcategory: "sc-bache", zone: "dn" },
  { desc: "Poste de luz apagado desde hace dos semanas en la calle Duarte, la zona queda muy oscura.", category: "alumbrado", subcategory: "sc-luminaria-apagada", zone: "sde" },
  { desc: "Acumulación de basura en el solar baldío junto al colmado, atrae insectos y mal olor.", category: "basura", subcategory: "sc-acumulacion", zone: "sdn" },
  { desc: "Alcantarilla destapada representa un peligro serio para los peatones y motoristas.", category: "alcantarillado", subcategory: "sc-tapa-faltante", zone: "sdo" },
  { desc: "Llevamos 5 días sin agua en el sector, varias familias no tienen cómo abastecerse.", category: "agua", subcategory: "sc-falta-suministro", zone: "alc" },
  { desc: "Vertido de aguas residuales está contaminando la cañada cercana al parque.", category: "ambiente", subcategory: "sc-canada", zone: "bch" },
  { desc: "Un colmado ha invadido la acera con mesas y sillas, obligando a peatones a caminar por la calle.", category: "ocupacion", subcategory: "sc-ocupacion-acera", zone: "dn" },
  { desc: "Semáforo dañado en un cruce peligroso cerca de la escuela primaria.", category: "calles", subcategory: "sc-semaforo", zone: "sde" },
  { desc: "Tapa de alcantarilla robada; ya hubo un motorista que casi cae dentro.", category: "alcantarillado", subcategory: "sc-tapa-faltante", zone: "dn" },
  { desc: "Fuga de agua potable corriendo por la calle desde hace tres días sin atención.", category: "agua", subcategory: "sc-fuga-potable", zone: "sdn" },
  { desc: "Zona sin alumbrado genera inseguridad para quienes regresan de noche.", category: "alumbrado", subcategory: "sc-luminaria-apagada", zone: "sdo" },
  { desc: "Basura acumulada frente al parque infantil, riesgo para los niños que juegan ahí.", category: "basura", subcategory: "sc-acumulacion", zone: "alc" },
  { desc: "La quema de basura a cielo abierto está afectando la respiración de varias familias.", category: "basura", subcategory: "sc-quema", zone: "pbr" },
  { desc: "Vendedores ambulantes ocupan el espacio público sin permiso, dificultando el paso.", category: "ocupacion", subcategory: "sc-venta-ambulante", zone: "sag" },
  { desc: "Calle completamente inundada tras la lluvia; queda intransitable por horas.", category: "calles", subcategory: "sc-inundacion", zone: "bch" },
  { desc: "Silla de un parque infantil está rota, con bordes filosos expuestos.", category: "mobiliario", subcategory: "sc-parque-danado", zone: "sde" },
  { desc: "Contenedor de basura desbordado desde hace más de una semana.", category: "basura", subcategory: "sc-contenedor", zone: "dn" },
  { desc: "Falta de presión de agua afecta a todo el residencial desde el fin de semana.", category: "agua", subcategory: "sc-baja-presion", zone: "sdo" },
  { desc: "Árbol caído bloquea parcialmente la vía principal tras la tormenta.", category: "ambiente", subcategory: "sc-arbol-caido", zone: "alc" },
  { desc: "Banca del parque comunitario deteriorada, con tablones rotos.", category: "mobiliario", subcategory: "sc-banca-danada", zone: "sag" },
  { desc: "Calle sin señalización adecuada ha provocado varios accidentes menores.", category: "calles", subcategory: "sc-senalizacion", zone: "sdn" },
  { desc: "Cableado eléctrico expuesto cerca del mercado municipal; vecinos están preocupados.", category: "alumbrado", subcategory: "sc-cableado", zone: "dn" },
  { desc: "Cañada acumula desechos plásticos y genera criaderos de mosquitos.", category: "ambiente", subcategory: "sc-canada", zone: "sde" },
  { desc: "Escombros de una demolición bloquean media calle desde hace días.", category: "ocupacion", subcategory: "sc-escombros", zone: "pbr" },
];
...
const STATUS_WEIGHTS = [
  ["nuevo", 8], ["en_revision", 6], ["requiere_info", 3], ["validado", 3], ["clasificado", 3],
  ["asignado", 8], ["en_gestion", 14], ["en_espera", 4], ["escalado", 3],
  ["pendiente_verificacion", 5], ["verificado", 3],
  ["resuelto", 6], ["cerrado", 24], ["rechazado", 3], ["duplicado", 2],
];
function pickWeightedStatus(rand) {
  const total = STATUS_WEIGHTS.reduce((s, [, w]) => s + w, 0);
  let roll = rand() * total;
  for (const [status, w] of STATUS_WEIGHTS) { if ((roll -= w) <= 0) return status; }
  return "cerrado";
}
...
const STAGE_ORDER_FOR_TASKS = ["asignado", "en_gestion", "en_espera", "escalado", "pendiente_verificacion", "verificado", "resuelto", "cerrado"];
...
const TASK_PROGRESS_BY_STATUS = {
  asignado: 0, en_gestion: 2, en_espera: 1, escalado: 2,
  pendiente_verificacion: 4, verificado: 5, resuelto: 5, cerrado: 5,
};
...
function buildTasksForReport(report, rand, now) {
  ...
  let t = report.dueAt ? report.dueAt - (PRIORITY_META[report.priority]?.slaHours || 72) * 3600000 : report.createdAt;
  return DEFAULT_TASK_TEMPLATE.map((tpl, i) => {
    ...
    const dependencyDone = !dependsOn || i < completedCount || DEFAULT_TASK_TEMPLATE.findIndex((x) => idOf(x.key) === dependsOn) < completedCount;
    let status;
    if (i < completedCount) status = "completada";
    else if (i === completedCount && (!dependsOn || dependencyDone)) status = "en_progreso";
...
const INSPECTABLE_CATEGORIES = ["calles", "alcantarillado", "alumbrado", "ambiente", "mobiliario"];
function buildInspectionsForReport(report, rand, now) {
  if (!STAGE_ORDER_FOR_TASKS.includes(report.status)) return [];
  if (!INSPECTABLE_CATEGORIES.includes(report.category)) return [];
  if (rand() > 0.55) return [];
  const requestedAt = Math.min((report.dueAt || now) - (PRIORITY_META[report.priority]?.slaHours || 72) * 3600000 / 2, now);
  const advanced = ["pendiente_verificacion", "verificado", "resuelto", "cerrado"].includes(report.status);
  ...
  if (advanced) {
    ...
    inspection.observations = inspection.result === "problema_no_encontrado"
    ...
    inspection.photos = [`https://picsum.photos/seed/${report.id}-insp/640/480`];
  }
  return [inspection];
}
...
function buildReports() {
  ...
  return REPORT_SEEDS.map((seed, i) => {
    ...
    if (priorityRoll > 0.88) priority = PRIORITY_ORDER[Math.min(PRIORITY_ORDER.indexOf(priority) + 1, 3)];
    else if (priorityRoll < 0.1) priority = PRIORITY_ORDER[Math.max(PRIORITY_ORDER.indexOf(priority) - 1, 0)];
    ...
    const satisfaction = status === "cerrado" && rand() > 0.4 ? 3 + Math.floor(rand() * 3) : null;
    ...
    const isAssignedStage = !["nuevo", "en_revision", "requiere_info", "validado", "clasificado"].includes(status);
    const department = isAssignedStage ? responsables[departmentIdx].department : null;
    const responsable = isAssignedStage ? responsables[departmentIdx].name : null;
    const dueAt = createdAt + slaHours * 3600000;

    const evidenceBefore = photos;
    const evidenceAfter = ["resuelto", "cerrado", "verificado"].includes(status) && rand() > 0.3
      ? [`https://picsum.photos/seed/tureporte-after-${i}/640/480`] : [];
    ...
    const timeline = [{ at: createdAt, label: "Reporte creado por el ciudadano", by: "Ciudadano", toStatus: "nuevo" }];
    let t = createdAt;
    const STAGE_ORDER = ["nuevo", "en_revision", "validado", "clasificado", "asignado", "en_gestion", "pendiente_verificacion", "verificado", "resuelto", "cerrado"];
    const currentIdx = STAGE_ORDER.indexOf(status);
    const passedThrough = currentIdx >= 0 ? STAGE_ORDER.slice(1, currentIdx + 1) : [];
    passedThrough.forEach((s) => {
      ...
      if (s === "en_revision") { label = "Reporte tomado en revisión"; by = "Gestor de incidencias"; }
      if (s === "validado") { label = "Reporte validado — la información es suficiente"; by = "Gestor de incidencias"; }
      if (s === "clasificado") { label = `Clasificado como ${getCategory(seed.category).label} → ${sub ? sub.label : "—"}`; by = "Gestor de incidencias"; }
      if (s === "asignado") { label = `Asignado a ${getInstitution(institutionId).short} · ${department || ""} · Responsable: ${responsable || "—"}`; by = "Gestor de incidencias"; }
      if (s === "en_gestion") { label = "Trabajo iniciado en sitio"; by = responsable || by; }
      if (s === "pendiente_verificacion") { label = "Marcado como listo para verificación"; by = responsable || by; }
      if (s === "verificado") { label = "Incidencia verificada en sitio"; by = "Supervisor"; }
      if (s === "resuelto") { label = "Incidencia marcada como resuelta"; by = "Supervisor"; }
      if (s === "cerrado") { label = "Incidencia cerrada"; by = "Sistema"; }
      timeline.push({ at: Math.min(t, now), label, by, toStatus: s });
    });
    if (status === "requiere_info") timeline.push({ at: createdAt + 3600000 * 5, label: "Se solicitó información adicional al ciudadano", by: "Gestor de incidencias", toStatus: "requiere_info", reason: "Las fotografías no muestran claramente la ubicación exacta." });
    if (status === "en_espera") timeline.push({ at: t + 3600000 * 4, label: "Incidencia marcada en espera", by: "Gestor de incidencias", toStatus: "en_espera", reason: "Se espera confirmación de acceso al predio." });
    if (status === "escalado") timeline.push({ at: t + 3600000 * 4, label: `Escalado a: ${ESCALATION_TARGETS[i % ESCALATION_TARGETS.length]}`, by: "Gestor de incidencias", toStatus: "escalado", reason: "Excede el tiempo de atención acordado (SLA)." });
    if (status === "rechazado") timeline.push({ at: createdAt + 3600000 * 6, label: "Reporte rechazado", by: "Gestor de incidencias", toStatus: "rechazado", reason: "No corresponde a una incidencia de infraestructura pública." });
    if (status === "duplicado") timeline.push({ at: createdAt + 3600000 * 6, label: "Marcado como posible duplicado de otro reporte activo", by: "Gestor de incidencias", toStatus: "duplicado", reason: "Ya existe un reporte activo para la misma ubicación y categoría." });
...
function formatDateTime(ts) {
  return new Date(ts).toLocaleString("es-DO", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
}
...
const isDueSoon = (r) => isActiveReport(r) && slaRemaining(r) >= 0 && slaRemaining(r) <= 6;
...
const isStagnant = (r) => isActiveReport(r) && hoursSinceActivity(r) >= STAGNANT_THRESHOLD_HOURS;
...
const NOT_YET_ASSIGNABLE_STATUSES = ["nuevo", "en_revision", "requiere_info", "rechazado", "duplicado"];
const isUnassigned = (r) => isActiveReport(r) && !r.responsable && !NOT_YET_ASSIGNABLE_STATUSES.includes(r.status);
...
const CASE_HEALTH_META = {
  healthy:  { label: "Healthy",  short: "Bajo control",         color: "#16a34a", dot: "bg-emerald-500", badge: "bg-emerald-50 text-emerald-700 border border-emerald-200" },
  at_risk:  { label: "At risk",  short: "Riesgo de incumplir",  color: "#d97706", dot: "bg-amber-500",   badge: "bg-amber-50 text-amber-700 border border-amber-200" },
  critical: { label: "Critical", short: "Requiere atención ya", color: "#dc2626", dot: "bg-red-500",     badge: "bg-red-50 text-red-700 border border-red-200" },
};
function getCaseHealth(report) {
  if (!isActiveReport(report)) return null;
  const reasons = [];
  if (isOverdue(report)) reasons.push("SLA vencido");
  if (isStagnant(report)) reasons.push(`${hoursSinceActivity(report)}h sin actividad`);
  if (["asignado", "en_gestion", "en_espera"].includes(report.status) && !report.responsable) reasons.push("Sin responsable asignado");
  if (report.status === "escalado") reasons.push("Escalado");
  if (reasons.length > 0) return { level: "critical", ...CASE_HEALTH_META.critical, reasons };

  const atRisk = [];
  if (isDueSoon(report)) atRisk.push("SLA próximo a vencer");
  if ((report.tasks || []).some((t) => t.dueAt && t.dueAt < Date.now() && t.status !== "completada")) atRisk.push("Tareas vencidas");
  if (report.priority === "critica" && report.status === "nuevo") atRisk.push("Crítica sin revisar");
  if (atRisk.length > 0) return { level: "at_risk", ...CASE_HEALTH_META.at_risk, reasons: atRisk };
...
const NOTIFY_STATUSES = new Set(["validado", "asignado", "requiere_info", "escalado", "pendiente_verificacion", "verificado", "resuelto", "cerrado", "en_gestion"]);
function buildNotificationsForUser(user, reports, rules) {
  if (!user) return [];
  const relevant = user.role === "ciudadano"
  ...
  relevant.forEach((r) => {
    ...
    if (last && last.toStatus && NOTIFY_STATUSES.has(last.toStatus)) {
      events.push({
        id: `${r.id}-${last.at}`, reportId: r.id, code: r.code,
        label: user.role === "ciudadano" ? citizenNotifLabel(last.toStatus, r) : last.label,
        at: last.at, tone: STATUS_META[last.toStatus]?.color || "#64748b",
      });
    }
    if (user.role !== "ciudadano" && notifySla) {
      if (isOverdue(r)) events.push({ id: `${r.id}-overdue`, reportId: r.id, code: r.code, label: `SLA vencido — ${r.desc.slice(0, 40)}…`, at: r.dueAt, tone: "#dc2626" });
      else if (isDueSoon(r)) events.push({ id: `${r.id}-duesoon`, reportId: r.id, code: r.code, label: `SLA próximo a vencer — ${r.desc.slice(0, 40)}…`, at: r.dueAt, tone: "#d97706" });
    }
  });
...
function citizenNotifLabel(status, r) {
  const map = {
    ...
    pendiente_verificacion: "El trabajo en tu reporte está pendiente de verificación",
    verificado: "La solución de tu reporte fue verificada",
    resuelto: "Tu reporte fue marcado como resuelto — confirma si el problema fue solucionado",
...
function buildAuditLog(reports, masterCases) {
  ...
  reports.forEach((r) => {
    ...
    (r.timeline || []).forEach((t, i) => {
      ...
      const anterior = hasStatusChange ? (runningStatus ? STATUS_META[runningStatus]?.label : "—") : "—";
      const nuevo = hasStatusChange ? (STATUS_META[t.toStatus]?.label || t.toStatus) : "—";
      if (hasStatusChange) runningStatus = t.toStatus;
      entries.push({
        id: `${r.id}-${i}`, at: t.at, by: t.by || "Sistema", action: t.label || "Actividad registrada",
        campo: hasStatusChange ? "Estado" : "Actividad", anterior, nuevo, motivo: t.reason || null,
        kind: "report", caseId: r.id, caseCode: r.code, institutionId: r.institutionId || null,
        category: r.category, zone: r.zone,
      });
    });
  });
  (masterCases || []).forEach((mc) => {
    (mc.timeline || []).forEach((t, i) => {
      entries.push({
        id: `${mc.id}-${i}`, at: t.at, by: t.by || "Sistema", action: t.label || "Actividad registrada",
        campo: "Caso maestro", anterior: "—", nuevo: "—", motivo: t.reason || null,
        kind: "masterCase", caseId: mc.id, caseCode: mc.code || mc.id, institutionId: null,
        category: mc.category, zone: mc.zone,
      });
    });
  });
...
function exportAuditCsv(entries, filename) {
  const header = ["Fecha", "Usuario", "Caso", "Acción", "Campo", "Valor anterior", "Valor nuevo", "Motivo"];
  const rows = entries.map((e) => [
    formatDateTime(e.at), `"${(e.by || "").replace(/"/g, "'")}"`, e.caseCode,
    `"${(e.action || "").replace(/"/g, "'")}"`, e.campo, e.anterior, e.nuevo,
    `"${(e.motivo || "").replace(/"/g, "'")}"`,
  ]);
...
function computeExecutiveKpis(reports) {
  ...
  if (!total) {
    return { frtHours: null, frtSample: 0, artHours: null, artSample: 0, slaCompliancePct: null, slaSample: 0, backlog: 0, agingHours: null, reopenRatePct: null, escalationRatePct: 0, resolutionRatePct: 0 };
  }
  const now = Date.now();
  ...
  reports.forEach((r) => {
    ...
    if (!tl.length) return;
    const first = tl[0].at;
    const firstMove = tl.slice(1).find((t) => t.toStatus && t.toStatus !== "nuevo");
    if (firstMove) frtSamples.push((firstMove.at - first) / 3600000);
  });
  const frtHours = frtSamples.length ? frtSamples.reduce((a, b) => a + b, 0) / frtSamples.length : null;

  const closed = reports.filter((r) => (r.status === "resuelto" || r.status === "cerrado") && (r.timeline || []).length > 1);
  const artSamples = closed.map((r) => {
    const tl = r.timeline;
    const closingEntry = [...tl].reverse().find((t) => t.toStatus === r.status) || tl[tl.length - 1];
    return (closingEntry.at - tl[0].at) / 3600000;
  });
  const artHours = artSamples.length ? artSamples.reduce((a, b) => a + b, 0) / artSamples.length : null;
  ...
  const slaCompliancePct = withSla.length ? (slaCompliant / withSla.length) * 100 : null;

  const activeReports = reports.filter(isActiveReport);
  const backlog = activeReports.length;
  const agingHours = activeReports.length ? activeReports.reduce((a, r) => a + (now - r.createdAt) / 3600000, 0) / activeReports.length : null;

  const closedEver = reports.filter((r) => r.status === "resuelto" || r.status === "cerrado" || (r.reopenedCount || 0) > 0).length;
  const reopened = reports.filter((r) => (r.reopenedCount || 0) > 0).length;
  const reopenRatePct = closedEver ? (reopened / closedEver) * 100 : null;

  const everEscalated = reports.filter((r) => (r.timeline || []).some((t) => t.toStatus === "escalado")).length;
  ...
  return {
    frtHours, frtSample: frtSamples.length,
    artHours, artSample: artSamples.length,
    slaCompliancePct, slaSample: withSla.length,
    backlog, agingHours,
    reopenRatePct, reopenSample: closedEver,
    escalationRatePct, resolutionRatePct,
  };
}
...
function CaseHealthBadge({ report, compact = false }) {
  ...
  return (
    ...
    >
      <span className={`w-1.5 h-1.5 rounded-full ${health.dot}`} /> {compact ? health.label : health.short}
    </span>
  );
}
...
function ZoneMap({ reports, selectedZone, onSelectZone, height = 380, mode, onModeChange, institutionFilter, hotspotZoneIds = [] }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
              ...
              >
                <button
                  ...
                  className={`relative w-full h-full rounded-[38%] flex flex-col items-center justify-center transition-all hover:scale-105 ${style.bg} ${style.text} ${style.border} ${
                    isSelected ? "ring-4 ring-blue-500 ring-offset-2" : isHotspot ? "ring-4 ring-red-400 ring-offset-1 animate-pulse" : ""
                  }`}
                >
                  {isHotspot && (
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-t border-slate-100 text-[11px] text-slate-500 flex-wrap">
        ...
        <span className="flex items-center gap-1 ml-1 pl-2 border-l border-slate-200"><Flame size={11} className="text-red-500" /> Sector caliente ({HOTSPOT_MIN_ACTIVE}+ activas en {HOTSPOT_WINDOW_DAYS}d)</span>
      </div>
    </div>
  );
}
...
/* Mapa operativo para gestor/administrador: filtros (categoría, prioridad,
   ...
   por zona (§19-20). */
function OperationalMap({ reports, institutionScope, mapMode, setMapMode, onOpenReport, title, eyebrow }) {
  ...
  const zoneConcentration = ZONES.map((z) => {
    ...
    inZone.forEach((r) => { byCat[r.category] = (byCat[r.category] || 0) + 1; });
    const topCat = Object.entries(byCat).sort((a, b) => b[1] - a[1])[0];
    return { zone: z, count: inZone.length, topCat: topCat ? { id: topCat[0], count: topCat[1] } : null };
  }).filter((z) => z.topCat && z.topCat.count >= 3).sort((a, b) => b.topCat.count - a.topCat.count);
  ...
  const scopedReports = reports.filter((r) => !institutionScope || r.institutionId === institutionScope);
  const hotspots = findHotspots(scopedReports);
  const hotspotZoneIds = hotspots.map((h) => h.zone.id);
  const selectedHotspot = selectedZone ? hotspots.find((h) => h.zone.id === selectedZone) : null;
  const selectedZoneReports = selectedZone
    ? filtered.filter((r) => r.zone === selectedZone).sort((a, b) => b.createdAt - a.createdAt).slice(0, 6)
    : [];
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pt-4 pb-6">
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <select value={catF} onChange={(e) => setCatF(e.target.value)} className="text-xs font-semibold rounded-full border border-slate-200 bg-white px-3 py-1.5 text-slate-600 outline-none">
          <option value="todas">Todas las categorías</option>
          {CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
        <select value={prioF} onChange={(e) => setPrioF(e.target.value)} className="text-xs font-semibold rounded-full border border-slate-200 bg-white px-3 py-1.5 text-slate-600 outline-none">
          <option value="todas">Toda prioridad</option>
          {PRIORITY_ORDER.map((p) => <option key={p} value={p}>{PRIORITY_META[p].label}</option>)}
        </select>
        <select value={statF} onChange={(e) => setStatF(e.target.value)} className="text-xs font-semibold rounded-full border border-slate-200 bg-white px-3 py-1.5 text-slate-600 outline-none">
          <option value="todas">Todo estado</option>
          {STATUS_ORDER.map((s) => <option key={s} value={s}>{STATUS_META[s].label}</option>)}
        </select>
        {!institutionScope && (
          <select value={instF} onChange={(e) => setInstF(e.target.value)} className="text-xs font-semibold rounded-full border border-slate-200 bg-white px-3 py-1.5 text-slate-600 outline-none">
            <option value="todas">Toda institución</option>
            {INSTITUTIONS.filter((i) => i.id !== "otra").map((i) => <option key={i.id} value={i.id}>{i.short}</option>)}
          </select>
        )}
      </div>

      <ZoneMap reports={filtered} selectedZone={selectedZone} onSelectZone={setSelectedZone} mode={mapMode} onModeChange={setMapMode} institutionFilter={institutionScope} height={360} hotspotZoneIds={hotspotZoneIds} />

      {hotspots.length > 0 && (
        <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4">
          <p className="text-sm font-bold text-red-800 mb-1 flex items-center gap-1.5"><Flame size={14} /> Sectores calientes</p>
          <p className="text-[11px] text-red-700 mb-3">
            Zonas con {HOTSPOT_MIN_ACTIVE} o más incidencias activas en los últimos {HOTSPOT_WINDOW_DAYS} días — concentración de carga que amerita atención territorial, sin importar la categoría. Toca un sector para verlo en el mapa.
          </p>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between rounded-xl border px-3 py-2 text-left transition ${
                    isSel ? "bg-red-600 border-red-600 text-white" : "bg-white border-red-100 hover:border-red-300"
                  }`}
                >
                  <span className="flex items-center gap-2">
                    <span className={`text-xs font-semibold ${isSel ? "text-white" : "text-slate-700"}`}>{h.zone.name}</span>
                    {h.critical > 0 && (
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isSel ? "bg-red-800 text-white" : "bg-red-100 text-red-700"}`}>
                        {h.critical} alta/crítica
                      </span>
                    )}
                  </span>
                  <span className={`text-xs font-bold ${isSel ? "text-white" : "text-red-700"}`}>
          ...
          {selectedHotspot && (
            <div className="mt-3 rounded-xl bg-white border border-red-100 p-3">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-bold text-slate-700">Incidencias recientes en {selectedHotspot.zone.name}</p>
                <button onClick={() => setSelectedZone(null)} className="text-[11px] font-semibold text-slate-400 hover:text-slate-600">Cerrar</button>
              </div>
              <div className="space-y-1.5">
                {selectedZoneReports.length === 0 && (
                  <p className="text-[11px] text-slate-400">Ningún reporte visible con los filtros actuales — ajusta categoría/prioridad/estado arriba.</p>
                )}
                {selectedZoneReports.map((r) => {
                  const meta = STATUS_META[r.status];
                  return (
                    <button key={r.id} onClick={() => onOpenReport(r.id)} className="w-full flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-50 text-left">
                      <span className="flex items-center gap-1.5 min-w-0">
                        <span className={`w-2 h-2 rounded-full shrink-0 ${getCategory(r.category).dot}`} />
                        <span className="text-[11px] font-semibold text-slate-700 truncate">{r.code}</span>
                        <span className="text-[11px] text-slate-400 truncate">{getCategory(r.category).label}</span>
                      </span>
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${meta.badge}`}>{meta.label}</span>
                    </button>
                  );
                })}
              </div>
      ...
      {zoneConcentration.length > 0 && (
        <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
          <p className="text-sm font-bold text-amber-800 mb-1 flex items-center gap-1.5"><AlertTriangle size={14} /> Problemas recurrentes detectados</p>
          <p className="text-[11px] text-amber-700 mb-3">Zonas con 3 o más incidencias de la misma categoría — posible problema estructural, no puntual.</p>
          <div className="space-y-2">
            {zoneConcentration.map((z) => (
              <div key={z.zone.id} className="flex items-center justify-between bg-white rounded-xl border border-amber-100 px-3 py-2">
                <span className="text-xs font-semibold text-slate-700">{z.zone.name}</span>
                <span className="text-xs font-bold text-amber-700">{z.topCat.count} × {getCategory(z.topCat.id).label}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <AuthShell onHome={onHome}>
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        ...
        <button onClick={() => onLogin(getUser("s1"))} className="rounded-xl border border-slate-200 bg-white py-3 px-1 flex flex-col items-center gap-1 hover:border-teal-400 hover:bg-teal-50 transition">
          <ShieldCheck size={18} className="text-fuchsia-700" /><span className="text-[11px] font-semibold text-slate-700">Supervisor</span>
        </button>
...
function AppShell({ user, view, setView, onLogout, children, unreadCount = 0 }) {
          ...
          {nav.map((n) => (
            <button
              ...
              className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition relative ${
                view === n.id ? "bg-sky-50 text-sky-700" : "text-slate-500 hover:bg-slate-50"
              }`}
            >
              <n.Icon size={18} /> {n.label}
              {n.id === "notify" && unreadCount > 0 && <span className="ml-auto text-[10px] font-bold bg-red-500 text-white rounded-full px-1.5 py-0.5">{unreadCount}</span>}
            </button>
          ))}
        </div>
        ...
        <header className="md:hidden sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-slate-200 px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setView("notify")} className="relative">
              ...
              {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-[8px] font-bold flex items-center justify-center">{unreadCount > 9 ? "9+" : unreadCount}</span>}
            </button>
          ...
          >
            {n.primary ? (
              ...
              <span className="relative">
                ...
                {n.id === "notify" && unreadCount > 0 && <span className="absolute -top-1 -right-1.5 w-2 h-2 rounded-full bg-red-500" />}
              </span>
            )}
...
function CitizenFeed({ reports, likedIds, onLike, onOpenProfile, onOpenDetail, currentUser }) {
  ...
  const list = useMemo(() => {
    const base = filter === "mias" ? reports.filter((r) => r.authorId === currentUser.id) : reports;
    const sorted = [...base].sort((a, b) => (filter === "cerca" ? a.distanceM - b.distanceM : filter === "mias" ? b.createdAt - a.createdAt : b.createdAt - a.createdAt));
    return sorted;
  ...
  return (
    <div className="max-w-xl mx-auto px-4 pt-4">
      ...
      <div className="flex rounded-full bg-slate-100 p-1 text-xs font-semibold w-fit mb-4 mt-4 flex-wrap">
        {[["cerca", "Cerca de ti"], ["recientes", "Más recientes"], ["mias", "Mis incidencias"]].map(([id, label]) => (
      ...
      <div className="space-y-4">
        ...
        {list.length === 0 && <p className="text-center text-sm text-slate-400 py-12">{filter === "mias" ? "Aún no has creado ningún reporte." : "No hay reportes en esta vista."}</p>}
      </div>
    </div>
  );
}
...
function CitizenComposer({ onPublish, showToast }) {
  ...
  const canPublish = category && subcategory && desc.trim().length > 8 && zone && photos.length > 0;

  const submit = () => {
    onPublish({ category, subcategory, desc, zone, anonymous, photos });
    setCategory(""); setSubcategory(""); setDesc(""); setZone(""); setAnonymous(false); setPhotos([]);
  ...
  return (
    <div className="max-w-xl mx-auto px-4 pt-4 pb-6">
      ...
      {category && (
        <label className="block mb-4">
          <span className="block text-xs font-semibold text-slate-600 mb-1.5">Tipo específico de incidencia</span>
          <select value={subcategory} onChange={(e) => setSubcategory(e.target.value)} className="w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-teal-400">
            <option value="">Selecciona el tipo exacto…</option>
            {subOptions.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
          </select>
          <p className="text-[10px] text-slate-400 mt-1">Elegir el tipo exacto acelera la asignación a la institución correcta.</p>
        </label>
      )}
...
function NotificationsView({ notifications, onOpenReport }) {
  return (
    <div className="max-w-2xl mx-auto px-4 pt-4 pb-8">
      ...
      <div className="space-y-2">
        {notifications.map((n) => (
          <button key={n.id} onClick={() => onOpenReport(n.reportId)} className="w-full text-left flex items-start gap-3 bg-white rounded-xl border border-slate-200 p-3 hover:border-sky-300 hover:shadow-sm transition">
            <span className="w-2.5 h-2.5 rounded-full mt-1.5 shrink-0" style={{ background: n.tone }} />
            <div className="min-w-0 flex-1">
...
const EVIDENCE_TONE_TEXT = { slate: "text-slate-500", indigo: "text-indigo-500", emerald: "text-emerald-600" };
function EvidenceGroup({ title, photos, tone = "slate", onAdd, addLabel }) {
  if (!photos?.length && !onAdd) return null;
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between mb-1.5">
        <p className={`text-[11px] font-bold uppercase tracking-wide ${EVIDENCE_TONE_TEXT[tone] || "text-slate-500"}`}>{title}</p>
        {onAdd && <button onClick={onAdd} className="text-[11px] font-semibold text-blue-700 flex items-center gap-1"><Camera size={11} /> {addLabel || "Agregar"}</button>}
      </div>
      {photos?.length ? (
        <div className="flex gap-2 overflow-x-auto tr-scroll pb-1">
          {photos.map((p, i) => (
            <img key={i} src={p} alt={title} className="w-24 h-24 rounded-lg object-cover shrink-0 border border-slate-200" />
          ))}
        </div>
      ) : (
        <div className="rounded-lg border border-dashed border-slate-200 py-4 text-center text-[11px] text-slate-400">Sin evidencia registrada todavía</div>
      )}
    </div>
  );
}
...
function CitizenProgressTracker({ report }) {
  if (report.status === "rechazado" || report.status === "duplicado") {
    ...
    return (
      <div className={`rounded-xl border p-3 mb-4 ${report.status === "rechazado" ? "bg-red-50 border-red-200" : "bg-rose-50 border-rose-200"}`}>
        <p className={`text-xs font-bold flex items-center gap-1.5 ${report.status === "rechazado" ? "text-red-700" : "text-rose-700"}`}>
          {report.status === "rechazado" ? <X size={13} /> : <Copy size={13} />} {report.status === "rechazado" ? "Este reporte fue rechazado" : "Marcado como duplicado de otro reporte activo"}
        </p>
        {entry?.reason && <p className="text-[11px] text-slate-600 mt-1">Motivo: {entry.reason}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 mb-4">
      <p className="text-[11px] font-bold text-slate-500 mb-2.5">¿Qué pasó con mi reporte?</p>
      <div className="space-y-2">
        {CITIZEN_STEPS.map((s) => {
          ...
          return (
            <div key={s.step} className="flex items-center gap-2">
              {done ? <CheckCircle2 size={16} className="text-emerald-600 shrink-0" /> : active ? <span className="w-4 h-4 rounded-full border-2 border-blue-600 shrink-0 relative"><span className="absolute inset-0.5 rounded-full bg-blue-600 animate-pulse" /></span> : <span className="w-4 h-4 rounded-full border-2 border-slate-300 shrink-0" />}
              <span className={`text-xs ${done ? "text-slate-500 line-through decoration-slate-300" : active ? "font-bold text-blue-700" : "text-slate-400"}`}>{s.label}</span>
            </div>
          );
        })}
      </div>
...
const TONE_BTN = {
  ...
  rose: "bg-rose-600 hover:bg-rose-700", cyan: "bg-cyan-600 hover:bg-cyan-700",
  indigo: "bg-indigo-600 hover:bg-indigo-700", fuchsia: "bg-fuchsia-600 hover:bg-fuchsia-700",
  purple: "bg-purple-600 hover:bg-purple-700", green: "bg-green-600 hover:bg-green-700",
  slate: "bg-slate-600 hover:bg-slate-700",
};

function ReportDetailModal({ report, comments, onAddComment, onClose, onOpenProfile, liked, onLike, session, allReports, onAction, onAddEvidence, onMergeDuplicate, onConfirmResolution, onAddTask, onUpdateTaskStatus, onRequestInspection, onRecordInspectionResult, rules }) {
  ...
  const [newTask, setNewTask] = useState({ title: "", description: "", assignee: "", priority: "media", dependsOn: "" });
  const [showInspectionForm, setShowInspectionForm] = useState(false);
  const [newInspection, setNewInspection] = useState({ assignee: "", scheduledFor: "" });
  ...
  const isOwnCitizenReport = session?.role === "ciudadano" && session.id === report.authorId;
  const actions = session ? availableActions(report, session.role) : [];
  const duplicates = canManage ? findPossibleDuplicates(report, allReports) : [];
  const responsablesForInst = getResponsablesFor(formInstitution || report.institutionId);
  ...
  const submitAction = () => {
    ...
    if (pendingAction.needsClassification) { payload.category = formCategory; payload.subcategory = formSubcategory; payload.priority = formPriority; }
    if (pendingAction.needsAssignment) { payload.institutionId = formInstitution; payload.department = formDepartment; payload.responsable = formResponsable; }
    if (pendingAction.needsEscalation) { payload.escalateTo = formEscalation; }
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center">
      <div className="bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-xl max-h-[94vh] overflow-y-auto tr-scroll tr-fade-up">
        <div className="sticky top-0 bg-white/95 backdrop-blur flex items-center justify-between px-4 py-3 border-b border-slate-100 z-10">
          <div>
            <span className="tr-mono text-sm font-bold text-slate-800">{report.code || `#${report.id}`}</span>
            <p className="text-[10px] text-slate-400">Expediente digital de la incidencia</p>
          </div>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mb-3">
            {report.isAnonymous ? (
              <div className="flex items-center gap-2"><Avatar anonymous size={34} /><span className="text-sm font-bold text-slate-800">Usuario anónimo</span></div>
            ) : (
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            ...
            {report.reopenedCount > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 border border-amber-200 px-2.5 py-1 text-xs font-semibold text-amber-700"><RefreshCw size={12} /> Reabierto {report.reopenedCount}×</span>}
          </div>
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 mb-3 text-xs text-slate-600 space-y-1.5">
            ...
            <div className="flex justify-between"><span className="flex items-center gap-1"><ListChecks size={12} /> Subcategoría</span><span className="font-semibold text-slate-800">{report.subcategory ? getSubcategory(report.subcategory)?.label : "Pendiente de clasificar"}</span></div>
            {active && (
              <div className="flex justify-between"><span className="flex items-center gap-1"><TimerReset size={12} /> SLA</span><span className={`font-semibold ${remaining < 0 ? "text-red-600" : remaining < 6 ? "text-orange-600" : "text-slate-800"}`}>{remaining < 0 ? `Vencido hace ${Math.abs(remaining)}h` : `${remaining}h restantes`}</span></div>
            )}
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-3 mb-3 text-xs space-y-1.5">
            <p className="text-[11px] font-bold text-blue-800 mb-1 flex items-center gap-1"><Building2 size={12} /> Asignación</p>
            <div className="flex justify-between"><span className="text-slate-500">Institución</span><span className="font-semibold text-slate-800">{getInstitution(report.institutionId).name}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Departamento</span><span className="font-semibold text-slate-800">{report.department || "Sin asignar aún"}</span></div>
            <div className="flex justify-between"><span className="text-slate-500">Responsable</span><span className="font-semibold text-slate-800">{report.responsable || "Sin asignar aún"}</span></div>
            {report.escalatedTo && <div className="flex justify-between"><span className="text-slate-500">Escalado a</span><span className="font-semibold text-fuchsia-700">{report.escalatedTo}</span></div>}
          </div>

          {canManage && duplicates.length > 0 && report.status !== "duplicado" && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 mb-3">
              <p className="text-[11px] font-bold text-amber-800 mb-2 flex items-center gap-1"><AlertTriangle size={12} /> Posible incidencia relacionada</p>
              <div className="space-y-1.5">
                {duplicates.map((d) => (
                  <div key={d.id} className="flex items-center justify-between bg-white rounded-lg border border-amber-100 px-2.5 py-1.5">
                    <span className="text-[11px] text-slate-600 truncate mr-2">{d.code} · {d.desc.slice(0, 40)}…</span>
                    <button onClick={() => onMergeDuplicate(report.id, d.id)} className="shrink-0 text-[11px] font-bold text-amber-700 hover:text-amber-900">Fusionar con este →</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          {canManage && (() => {
            const recurring = findRecurringProblems(report, allReports, rules);
            return recurring.length > 0 ? (
              <div className="rounded-xl border border-purple-200 bg-purple-50 p-3 mb-3">
                <p className="text-[11px] font-bold text-purple-800 flex items-center gap-1"><TrendingUp size={12} /> Problema recurrente</p>
                <p className="text-[11px] text-purple-700 mt-0.5">{recurring.length} incidencias de {getCategory(report.category).label.toLowerCase()} en {getZone(report.zone).name} en los últimos 30 días — posible problema estructural.</p>
              </div>
            ) : null;
          })()}
          ...
          <EvidenceGroup title="Antes (reporte inicial)" photos={report.evidenceBefore || report.photos} tone="slate" />
          <EvidenceGroup title="Durante (gestión)" photos={report.evidenceDuring} tone="indigo" onAdd={canManage ? () => onAddEvidence(report.id, `https://picsum.photos/seed/${uid("ev")}/640/480`, "during") : null} addLabel="Agregar evidencia de gestión" />
          <EvidenceGroup title="Después (resolución)" photos={report.evidenceAfter} tone="emerald" onAdd={canManage ? () => onAddEvidence(report.id, `https://picsum.photos/seed/${uid("ev")}/640/480`, "after") : null} addLabel="Agregar evidencia de resolución" />
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-bold text-slate-500 flex items-center gap-1">
                  ...
                  {report.tasks && report.tasks.length > 0 && (
                    <span className="text-slate-400 font-normal">· {report.tasks.filter((t) => t.status === "completada").length}/{report.tasks.length} completadas</span>
                  )}
                </p>
                <button onClick={() => setShowTaskForm((s) => !s)} className="flex items-center gap-1 text-[11px] font-bold text-teal-700 hover:text-teal-900"><Plus size={12} /> Agregar tarea</button>
              </div>

              {(!report.tasks || report.tasks.length === 0) && !showTaskForm && (
                <div className="rounded-lg border border-dashed border-slate-200 py-3 text-center text-[11px] text-slate-400">Sin tareas creadas todavía</div>
              )}

              <div className="space-y-1.5">
                {(report.tasks || []).map((t) => {
                  const meta = TASK_STATUS_META[t.status];
                  const dep = t.dependsOn ? (report.tasks || []).find((x) => x.id === t.dependsOn) : null;
                  return (
                    <div key={t.id} className="rounded-lg border border-slate-200 bg-white p-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2 min-w-0">
                          <meta.Icon size={14} style={{ color: meta.color }} className="mt-0.5 shrink-0" />
                          <div className="min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-1.5 mt-1">
                              <span className={`inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${meta.badge}`}>{meta.label}</span>
                              {t.assignee && <span className="text-[10px] text-slate-400">{t.assignee}</span>}
                              {dep && t.status === "bloqueada" && <span className="text-[10px] text-red-500">Bloqueada por: "{dep.title}"</span>}
                            </div>
                          </div>
                        </div>
                        <div className="shrink-0 flex gap-1">
                          {t.status === "pendiente" && (
                            <button onClick={() => onUpdateTaskStatus(report.id, t.id, "en_progreso")} className="text-[10px] font-bold text-blue-600 hover:text-blue-800 whitespace-nowrap">Iniciar</button>
                          )}
                          {t.status === "en_progreso" && (
                            <button onClick={() => onUpdateTaskStatus(report.id, t.id, "completada")} className="text-[10px] font-bold text-emerald-600 hover:text-emerald-800 whitespace-nowrap">Completar</button>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              {showTaskForm && (
                <div className="mt-2 rounded-lg bg-slate-50 border border-slate-200 p-3 space-y-2">
                  <input value={newTask.title} onChange={(e) => setNewTask((f) => ({ ...f, title: e.target.value }))} placeholder="Título de la tarea" className="w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white outline-none focus:ring-2 focus:ring-teal-400" />
                  <textarea value={newTask.description} onChange={(e) => setNewTask((f) => ({ ...f, description: e.target.value }))} placeholder="Descripción (opcional)" rows={2} className="w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white outline-none focus:ring-2 focus:ring-teal-400 resize-none" />
                  <div className="grid grid-cols-2 gap-2">
                    <label className="text-[11px] text-slate-500">Responsable
                      <select value={newTask.assignee} onChange={(e) => setNewTask((f) => ({ ...f, assignee: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                        <option value="">Sin asignar</option>
                        {responsablesForInst.map((r) => <option key={r.id} value={r.name}>{r.name} · {r.department}</option>)}
                      </select>
                    </label>
                    <label className="text-[11px] text-slate-500">Prioridad
                      <select value={newTask.priority} onChange={(e) => setNewTask((f) => ({ ...f, priority: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                        {PRIORITY_ORDER.map((p) => <option key={p} value={p}>{PRIORITY_META[p].label}</option>)}
                      </select>
                    </label>
                  </div>
                  {report.tasks && report.tasks.length > 0 && (
                    <label className="text-[11px] text-slate-500 block">No puede iniciar hasta completar
                      <select value={newTask.dependsOn} onChange={(e) => setNewTask((f) => ({ ...f, dependsOn: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                        <option value="">Ninguna (puede iniciar de inmediato)</option>
                        {report.tasks.map((t) => <option key={t.id} value={t.id}>{t.title}</option>)}
                      </select>
                    </label>
                  )}
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setShowTaskForm(false)} className="flex-1 rounded-lg border border-slate-200 py-1.5 text-xs font-semibold text-slate-500">Cancelar</button>
                    <button
                      onClick={() => { if (newTask.title.trim()) { onAddTask(report.id, newTask); setNewTask({ title: "", description: "", assignee: "", priority: "media", dependsOn: "" }); setShowTaskForm(false); } }}
          ...
          {canManage && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-bold text-slate-500 flex items-center gap-1"><ClipboardCheck size={12} /> Inspecciones</p>
                <button onClick={() => setShowInspectionForm((s) => !s)} className="flex items-center gap-1 text-[11px] font-bold text-teal-700 hover:text-teal-900"><Plus size={12} /> Solicitar inspección</button>
              </div>

              {(!report.inspections || report.inspections.length === 0) && !showInspectionForm && (
                <div className="rounded-lg border border-dashed border-slate-200 py-3 text-center text-[11px] text-slate-400">Sin inspecciones solicitadas todavía</div>
              )}

              <div className="space-y-1.5">
                {(report.inspections || []).map((insp) => {
                  const resultMeta = insp.result ? INSPECTION_RESULT_META[insp.result] : null;
                  const draft = inspectionResultDraft[insp.id] || { result: "", observations: "" };
                  return (
                    <div key={insp.id} className="rounded-lg border border-slate-200 bg-white p-2.5">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${insp.status === "realizada" ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-amber-100 text-amber-700 border border-amber-200"}`}>
                          {insp.status === "realizada" ? "Realizada" : "Solicitada"}
                        </span>
                        {insp.assignee && <span className="text-[10px] text-slate-400">{insp.assignee}</span>}
                      </div>
                      {insp.scheduledFor && insp.status === "solicitada" && (
                        <p className="text-[11px] text-slate-500 flex items-center gap-1"><CalendarClock size={11} /> Programada: {formatDateTime(insp.scheduledFor)}</p>
                      )}
                      {insp.status === "realizada" && (
                        <>
                          <span className={`inline-block mt-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${resultMeta.badge}`}>{resultMeta.label}</span>
                          {insp.observations && <p className="text-[11px] text-slate-500 mt-1">{insp.observations}</p>}
                        </>
                      )}
                      {insp.status === "solicitada" && (
                        <div className="mt-2 pt-2 border-t border-slate-100 space-y-1.5">
                          <select value={draft.result} onChange={(e) => setInspectionResultDraft((m) => ({ ...m, [insp.id]: { ...draft, result: e.target.value } }))} className="w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                            <option value="">Registrar resultado…</option>
                            {INSPECTION_RESULT_ORDER.map((k) => <option key={k} value={k}>{INSPECTION_RESULT_META[k].label}</option>)}
                          </select>
                          {draft.result && (
                            <>
                              <textarea value={draft.observations} onChange={(e) => setInspectionResultDraft((m) => ({ ...m, [insp.id]: { ...draft, observations: e.target.value } }))} placeholder="Observaciones de la visita" rows={2} className="w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white resize-none" />
                              <button
                                onClick={() => { onRecordInspectionResult(report.id, insp.id, draft); setInspectionResultDraft((m) => ({ ...m, [insp.id]: { result: "", observations: "" } })); }}
              ...
              {showInspectionForm && (
                <div className="mt-2 rounded-lg bg-slate-50 border border-slate-200 p-3 space-y-2">
                  <label className="text-[11px] text-slate-500 block">Responsable de la inspección
                    <select value={newInspection.assignee} onChange={(e) => setNewInspection((f) => ({ ...f, assignee: e.target.value }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                      <option value="">Sin asignar</option>
                      {responsablesForInst.map((r) => <option key={r.id} value={r.name}>{r.name} · {r.department}</option>)}
                    </select>
                  </label>
                  <label className="text-[11px] text-slate-500 block">Fecha programada
                    <input type="datetime-local" onChange={(e) => setNewInspection((f) => ({ ...f, scheduledFor: e.target.value ? new Date(e.target.value).getTime() : null }))} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white" />
                  </label>
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setShowInspectionForm(false)} className="flex-1 rounded-lg border border-slate-200 py-1.5 text-xs font-semibold text-slate-500">Cancelar</button>
                    <button
                      onClick={() => { onRequestInspection(report.id, newInspection); setNewInspection({ assignee: "", scheduledFor: "" }); setShowInspectionForm(false); }}
          ...
          {isOwnCitizenReport && report.status === "cerrado" && !report.citizenConfirmed && (
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 mb-4">
              <p className="text-xs font-bold text-blue-800 mb-2">¿El problema fue solucionado?</p>
              <div className="flex gap-2">
                <button onClick={() => onConfirmResolution(report.id, true)} className="flex-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-2">Sí, está solucionado</button>
                <button onClick={() => onConfirmResolution(report.id, false)} className="flex-1 rounded-lg bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-2">No, continúa el problema</button>
              </div>
            </div>
          )}
          {isOwnCitizenReport && report.citizenConfirmed && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-2.5 mb-4 text-[11px] font-semibold text-emerald-700 flex items-center gap-1.5"><CheckCircle2 size={13} /> Confirmaste que el problema fue solucionado.</div>
          )}

          <div className="mb-4">
            <p className="text-xs font-bold text-slate-500 mb-2">Línea de tiempo del expediente</p>
            <div className="space-y-2">
              {report.timeline.map((t, i) => (
                <div key={i} className="flex gap-2 text-xs">
                  <div className="flex flex-col items-center pt-0.5">
                    <span className="w-2 h-2 rounded-full" style={{ background: t.toStatus ? STATUS_META[t.toStatus]?.color : "#0369a1" }} />
                  ...
                  <div className="pb-2">
                    ...
                    <p className="text-slate-400">{t.by} · {formatDateTime(t.at)}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {session && actions.length > 0 && (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 mb-4">
              <p className="text-xs font-bold text-slate-600 mb-2.5 flex items-center gap-1"><ClipboardList size={13} /> Acciones disponibles para {ROLE_META[session.role].label}</p>
              ...
              {pendingAction && (
                <div className="mt-3 rounded-lg bg-white border border-slate-200 p-3 space-y-2.5">
                  <p className="text-xs font-bold text-slate-700">{pendingAction.label}</p>

                  {pendingAction.needsClassification && (
                    <div className="grid grid-cols-2 gap-2">
                      <label className="text-[11px] text-slate-500">Categoría
                        <select value={formCategory} onChange={(e) => { setFormCategory(e.target.value); setFormSubcategory(""); }} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                          {CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
                        </select>
                      </label>
                      <label className="text-[11px] text-slate-500">Subcategoría
                        <select value={formSubcategory} onChange={(e) => setFormSubcategory(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                          <option value="">Selecciona…</option>
                          {getSubcategoriesFor(formCategory).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
                        </select>
                      </label>
                      <label className="text-[11px] text-slate-500 col-span-2">Prioridad
                        <select value={formPriority} onChange={(e) => setFormPriority(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                          {PRIORITY_ORDER.map((p) => <option key={p} value={p}>{PRIORITY_META[p].label} · SLA {PRIORITY_META[p].slaHours}h</option>)}
                        </select>
                      </label>
                    </div>
                  )}
                  ...
                  {pendingAction.needsAssignment && (
                    <div className="grid grid-cols-1 gap-2">
                      <label className="text-[11px] text-slate-500">Institución responsable
                        <select value={formInstitution} onChange={(e) => { setFormInstitution(e.target.value); setFormDepartment(""); setFormResponsable(""); }} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                          {INSTITUTIONS.filter((i) => i.id !== "otra").map((i) => <option key={i.id} value={i.id}>{i.name}</option>)}
                        </select>
                      </label>
                      <div className="grid grid-cols-2 gap-2">
                        <label className="text-[11px] text-slate-500">Departamento
                          <select value={formDepartment} onChange={(e) => setFormDepartment(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                            <option value="">Selecciona…</option>
                            {getDepartmentsFor(formInstitution).map((d) => <option key={d} value={d}>{d}</option>)}
                          </select>
                        </label>
                        <label className="text-[11px] text-slate-500">Responsable
                          <select value={formResponsable} onChange={(e) => setFormResponsable(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                            <option value="">Selecciona…</option>
                            {responsablesForInst.map((r) => <option key={r.id} value={r.name}>{r.name} · {r.department}</option>)}
                          </select>
                        </label>
                  ...
                  {pendingAction.needsEscalation && (
                    <label className="text-[11px] text-slate-500 block">Escalar a
                      <select value={formEscalation} onChange={(e) => setFormEscalation(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
                  ...
                  {pendingAction.reason && (
                    <label className="text-[11px] text-slate-500 block">{pendingAction.reasonLabel || "Motivo (obligatorio)"}
                      <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={2} className="mt-1 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white outline-none focus:ring-2 focus:ring-blue-400" placeholder="Explica brevemente el motivo…" />
                    </label>
                  )}

                  <div className="flex gap-2 pt-1">
                    <button onClick={() => setPendingAction(null)} className="flex-1 rounded-lg border border-slate-200 py-1.5 text-xs font-semibold text-slate-500">Cancelar</button>
                    <button
            ...
            {comments.map((c) => (
              <div key={c.id} className="flex gap-2">
                ...
                <div className="bg-slate-50 rounded-xl px-3 py-2 flex-1">
                  <p className="text-xs font-bold text-slate-700">{c.anonymous ? "Usuario anónimo" : c.name}</p>
...
function GestorDashboard({ reports, user, onOpenDetail, showToast }) {
  ...
  const TABS = [
    ...
    { id: "validacion", label: "Por validar", test: (r) => ["en_revision", "requiere_info"].includes(r.status) },
    { id: "asignadas", label: "Asignadas", test: (r) => ["validado", "clasificado", "asignado"].includes(r.status) },
    { id: "gestion", label: "En gestión", test: (r) => ["en_gestion", "en_espera"].includes(r.status) },
    { id: "escaladas", label: "Escaladas", test: (r) => r.status === "escalado" },
    { id: "verificacion", label: "Por verificar", test: (r) => r.status === "pendiente_verificacion" },
    ...
    { id: "reabiertas", label: "Reabiertas", test: (r) => (r.reopenedCount || 0) > 0 && isActiveReport(r) },
    { id: "cerradas", label: "Cerradas", test: (r) => ["resuelto", "cerrado"].includes(r.status) },
  ...
  const filtered = mine
    ...
    .sort((a, b) => (a.priority === "critica" ? -1 : 1) - (b.priority === "critica" ? -1 : 1));

  /* Los 8 indicadores del Operations Center — mismo orden que el brief §25. */
  const indicators = [
    { id: "vencidas", label: "Vencidos", value: mine.filter((r) => isOverdue(r)).length, tone: "red" },
    { id: "validacion", label: "SLA por vencer", value: mine.filter((r) => isDueSoon(r)).length, tone: "amber" },
    { id: "sin_asignar", label: "Sin asignar", value: mine.filter((r) => isUnassigned(r)).length, tone: "slate" },
    { id: "estancadas", label: "Estancados", value: mine.filter((r) => isStagnant(r)).length, tone: "orange" },
    { id: "escaladas", label: "Escalados", value: mine.filter((r) => r.status === "escalado").length, tone: "fuchsia" },
    { id: "verificacion", label: "Verificación", value: mine.filter((r) => r.status === "pendiente_verificacion").length, tone: "purple" },
    { id: "reabiertas", label: "Reabiertos", value: mine.filter((r) => (r.reopenedCount || 0) > 0 && isActiveReport(r)).length, tone: "indigo" },
    { id: "cerradas", label: "Cerrados", value: mine.filter((r) => ["resuelto", "cerrado"].includes(r.status)).length, tone: "emerald" },
  ];
  const TONE_CLASSES = {
    red: "border-red-200 bg-red-50 text-red-600 [&_.tr-ind-label]:text-red-400",
    amber: "border-amber-200 bg-amber-50 text-amber-600 [&_.tr-ind-label]:text-amber-500",
    slate: "border-slate-200 bg-white text-slate-800 [&_.tr-ind-label]:text-slate-400",
    orange: "border-orange-200 bg-orange-50 text-orange-600 [&_.tr-ind-label]:text-orange-500",
    fuchsia: "border-fuchsia-200 bg-fuchsia-50 text-fuchsia-700 [&_.tr-ind-label]:text-fuchsia-500",
    purple: "border-purple-200 bg-purple-50 text-purple-700 [&_.tr-ind-label]:text-purple-500",
    indigo: "border-indigo-200 bg-indigo-50 text-indigo-700 [&_.tr-ind-label]:text-indigo-500",
    emerald: "border-emerald-200 bg-emerald-50 text-emerald-700 [&_.tr-ind-label]:text-emerald-500",
  };

  return (
    <div className="max-w-3xl mx-auto px-4 pt-4 pb-6">
      <div className="flex items-start justify-between gap-3 mb-1">
        <SectionHeading eyebrow={getInstitution(user.institutionId).name} title="Centro de operaciones" />
        <button
      ...
      <div className="grid grid-cols-4 gap-2 mb-4">
        {indicators.map((ind) => (
          ...
          >
            <p className="tr-mono text-lg font-bold leading-none">{ind.value}</p>
            <p className="tr-ind-label text-[10px] font-semibold mt-1">{ind.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <div className="flex rounded-full bg-slate-100 p-1 text-xs font-semibold flex-wrap">
          {TABS.map((t) => (
            <button key={t.id} onClick={() => setTab(t.id)} className={`px-3 py-1.5 rounded-full transition whitespace-nowrap ${tab === t.id ? "bg-white shadow text-slate-900" : "text-slate-500"}`}>{t.label}</button>
          ))}
        </div>
      </div>
      <div className="flex items-center gap-2 mb-3 flex-wrap">
      ...
      <div className="space-y-2">
        {filtered.map((r) => {
          ...
          return (
            <button key={r.id} onClick={() => onOpenDetail(r)} className="w-full text-left flex items-center gap-3 bg-white rounded-xl border border-slate-200 p-3 hover:border-sky-300 hover:shadow-sm transition">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap"><CategoryPill id={r.category} /><StatusPill status={r.status} /><PriorityPill priority={r.priority} /><CaseHealthBadge report={r} compact /></div>
              ...
              <div className="text-right shrink-0">
                {isActiveReport(r) && <p className={`text-[11px] font-bold ${remaining < 0 ? "text-red-600" : remaining < 24 ? "text-orange-500" : "text-slate-400"}`}>{remaining < 0 ? "Vencido" : `${remaining}h`}</p>}
                <ChevronRight size={16} className="text-slate-300 ml-auto mt-1" />
              </div>
            </button>
          );
        })}
        {filtered.length === 0 && <p className="text-center text-sm text-slate-400 py-10">No hay incidencias en esta vista.</p>}
      </div>
    </div>
  );
}
...
function SupervisorSection({ title, count, tone = "slate", children, empty }) {
  const TONE = {
    slate: "text-slate-700", red: "text-red-600", amber: "text-amber-600",
    orange: "text-orange-600", fuchsia: "text-fuchsia-600", indigo: "text-indigo-600",
  };
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-3">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-slate-800">{title}</h3>
        <span className={`tr-mono text-sm font-bold ${TONE[tone]}`}>{count}</span>
      </div>
      {count === 0 ? <p className="text-xs text-slate-400 py-2">{empty}</p> : <div className="space-y-1.5">{children}</div>}
    </div>
  );
}
function SupervisorCaseRow({ r, onOpenDetail, extra }) {
  return (
    <button onClick={() => onOpenDetail(r)} className="w-full text-left flex items-center gap-2.5 rounded-lg px-2 py-1.5 hover:bg-slate-50 transition">
      <img src={r.photos[0]} className="w-9 h-9 rounded-lg object-cover shrink-0" alt="" />
      <div className="min-w-0 flex-1">
        <p className="text-xs font-semibold text-slate-700 truncate">{r.code} · {r.desc}</p>
        <p className="text-[10px] text-slate-400">{getZone(r.zone).name} · {r.responsable || "Sin responsable"}</p>
      </div>
      {extra}
...
function SupervisorControlCenter({ reports, user, onOpenDetail, masterCases, onCreateMasterCase, onOpenMasterCase }) {
  const mine = reports.filter((r) => r.institutionId === user.institutionId);
  ...
  const reopened = mine.filter((r) => (r.reopenedCount || 0) > 0 && isActiveReport(r));
  ...
  const workload = responsables.map((resp) => {
    const assigned = mine.filter((r) => r.responsable === resp.name && isActiveReport(r));
    return {
      ...resp,
      active: assigned.length,
      critical: assigned.filter((r) => getCaseHealth(r)?.level === "critical").length,
  ...
  const recurringGroups = useMemo(() => {
    ...
    return Object.entries(groups).filter(([, list]) => list.length >= 3).map(([key, list]) => {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pt-4 pb-6">
      <SectionHeading eyebrow={getInstitution(user.institutionId).name} title="Centro de supervisión" />

      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="rounded-xl border border-red-200 bg-red-50 p-2.5"><p className="tr-mono text-lg font-bold text-red-600">{critical.length}</p><p className="text-[10px] text-red-400 font-semibold">Críticos</p></div>
        <div className="rounded-xl border border-orange-200 bg-orange-50 p-2.5"><p className="tr-mono text-lg font-bold text-orange-600">{stagnant.length}</p><p className="text-[10px] text-orange-400 font-semibold">Estancados</p></div>
        <div className="rounded-xl border border-fuchsia-200 bg-fuchsia-50 p-2.5"><p className="tr-mono text-lg font-bold text-fuchsia-700">{escalated.length}</p><p className="text-[10px] text-fuchsia-500 font-semibold">Escalados</p></div>
      </div>

      <SupervisorSection title="Casos críticos" count={critical.length} tone="red" empty="No hay casos críticos en este momento.">
        {critical.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} extra={<CaseHealthBadge report={r} compact />} />)}
      </SupervisorSection>

      <SupervisorSection title="SLA vencidos" count={overdue.length} tone="red" empty="No hay SLA vencidos.">
        {overdue.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} extra={<span className="text-[10px] font-bold text-red-600 shrink-0">{Math.abs(slaRemaining(r))}h vencido</span>} />)}
      </SupervisorSection>

      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-3">
        <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2"><Users size={15} className="text-slate-400" /> Carga de trabajo por responsable</h3>
        <div className="space-y-2.5">
          {workload.map((w) => (
            <div key={w.id} className="flex items-center gap-3">
              <div className="w-28 shrink-0">
                <p className="text-xs font-semibold text-slate-700 truncate">{w.name}</p>
                <p className="text-[10px] text-slate-400 truncate">{w.department}</p>
              </div>
              <div className="flex-1">
                <ProgressBar value={(w.active / maxLoad) * 100} colorClass={w.overdue > 0 ? "bg-red-500" : w.active >= maxLoad * 0.8 ? "bg-amber-500" : "bg-teal-500"} />
              </div>
              <div className="text-right shrink-0 w-24 text-[10px] font-semibold text-slate-500">
                {w.active} activos {w.critical > 0 && <span className="text-red-500">· {w.critical} crít.</span>} {w.overdue > 0 && <span className="text-red-500">· {w.overdue} venc.</span>}
              </div>
            </div>
          ))}
          {workload.length === 0 && <p className="text-xs text-slate-400">No hay responsables configurados para esta institución.</p>}
        </div>
      </div>

      <SupervisorSection title="Casos sin responsable" count={unassigned.length} tone="slate" empty="Todos los casos asignables tienen responsable.">
        {unassigned.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} />)}
      </SupervisorSection>

      <SupervisorSection title="Casos estancados" count={stagnant.length} tone="orange" empty="No hay casos sin actividad reciente.">
        {stagnant.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} extra={<span className="text-[10px] font-bold text-orange-600 shrink-0">{hoursSinceActivity(r)}h sin actividad</span>} />)}
      </SupervisorSection>

      <SupervisorSection title="Escalaciones activas" count={escalated.length} tone="fuchsia" empty="No hay casos escalados.">
        {escalated.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} extra={<span className="text-[10px] font-semibold text-fuchsia-600 shrink-0">{r.escalatedTo || "—"}</span>} />)}
      </SupervisorSection>

      <SupervisorSection title="Reaperturas" count={reopened.length} tone="indigo" empty="No hay casos reabiertos activos.">
        {reopened.slice(0, 8).map((r) => <SupervisorCaseRow key={r.id} r={r} onOpenDetail={onOpenDetail} extra={<span className="text-[10px] font-semibold text-indigo-600 shrink-0">{r.reopenedCount}× reabierto</span>} />)}
      </SupervisorSection>

      <div className="bg-white rounded-2xl border border-slate-200 p-4">
        <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2"><TrendingUp size={15} className="text-purple-500" /> Problemas recurrentes detectados</h3>
        {recurringGroups.length === 0 && <p className="text-xs text-slate-400 py-2">No se detectaron patrones recurrentes en los últimos 30 días.</p>}
        <div className="space-y-2">
          {recurringGroups.map((g) => {
            const existing = masterCases.find((mc) => mc.category === g.category && mc.zone === g.zone && mc.status !== "cerrado");
            return (
              <div key={`${g.category}-${g.zone}`} className="rounded-xl border border-purple-200 bg-purple-50 p-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-xs font-bold text-purple-800">{g.list.length} incidencias de {getCategory(g.category).label} · {getZone(g.zone).name}</p>
                    <p className="text-[11px] text-purple-600 mt-0.5">Últimos 30 días — posible problema de infraestructura, no casos aislados.</p>
                  </div>
                  {existing ? (
                    <button onClick={() => onOpenMasterCase(existing.id)} className="shrink-0 text-[10px] font-bold bg-purple-600 text-white rounded-full px-2.5 py-1 hover:bg-purple-700">Ver {existing.code}</button>
                  ) : (
                    <button onClick={() => onCreateMasterCase(g.category, g.zone, g.list.map((r) => r.id))} className="shrink-0 text-[10px] font-bold bg-white border border-purple-300 text-purple-700 rounded-full px-2.5 py-1 hover:bg-purple-100">Crear caso maestro</button>
                  )}
                </div>
                <div className="flex gap-1.5 mt-2 flex-wrap">
                  {g.list.slice(0, 5).map((r) => (
                    <button key={r.id} onClick={() => onOpenDetail(r)} className="text-[10px] tr-mono font-semibold bg-white border border-purple-200 text-purple-700 rounded-full px-2 py-0.5 hover:bg-purple-100">{r.code}</button>
                  ))}
                </div>
              </div>
            );
...
const MASTER_CASE_STATUS_META = {
  abierto:         { label: "Abierto",          badge: "bg-slate-100 text-slate-700 border border-slate-200" },
  en_intervencion: { label: "En intervención",  badge: "bg-indigo-100 text-indigo-700 border border-indigo-200" },
  cerrado:         { label: "Cerrado",          badge: "bg-emerald-100 text-emerald-700 border border-emerald-200" },
};
...
const INTERVENTION_ACTION_TEMPLATE = ["Inspección", "Limpieza", "Reparación", "Mantenimiento", "Verificación"];

function MasterCasesView({ masterCases, onOpen }) {
  return (
    <div className="max-w-3xl mx-auto px-4 pt-4 pb-6">
      <SectionHeading eyebrow="Problemas comunitarios agrupados" title="Casos maestros" />
      {masterCases.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 text-center">
          <Layers size={28} className="text-slate-300 mx-auto mb-2" />
          <p className="text-sm text-slate-500">Aún no hay casos maestros creados.</p>
          <p className="text-xs text-slate-400 mt-1">Se crean desde "Problemas recurrentes detectados" en el Centro de supervisión.</p>
        </div>
      )}
      <div className="space-y-2.5">
        {masterCases.map((mc) => {
          ...
          return (
            <button key={mc.id} onClick={() => onOpen(mc.id)} className="w-full text-left bg-white rounded-2xl border border-slate-200 p-4 hover:border-purple-300 hover:shadow-sm transition">
              <div className="flex items-center justify-between mb-1.5">
                <span className="tr-mono text-xs font-bold text-slate-800">{mc.code}</span>
                <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${MASTER_CASE_STATUS_META[mc.status].badge}`}>{MASTER_CASE_STATUS_META[mc.status].label}</span>
              </div>
              <p className="text-sm font-semibold text-slate-700">{getCategory(mc.category).label} · {getZone(mc.zone).name}</p>
              <p className="text-xs text-slate-400 mt-0.5">{mc.reportIds.length} incidencias agrupadas</p>
              {plan && (
                <div className="mt-2.5">
                  <div className="flex items-center justify-between text-[10px] text-slate-500 mb-1"><span>Plan de intervención</span><span>{doneActions}/{plan.actions.length} acciones</span></div>
                  <ProgressBar value={(doneActions / plan.actions.length) * 100} colorClass="bg-indigo-500" />
                </div>
              )}
            </button>
          );
...
function InterventionPlanForm({ masterCase, onCreate }) {
  const [problem, setProblem] = useState(`${getCategory(masterCase.category).label} recurrente en ${getZone(masterCase.zone).name}`);
  const [objective, setObjective] = useState(`Reducir las incidencias de ${getCategory(masterCase.category).label.toLowerCase()} en la zona`);
  const [actions, setActions] = useState([...INTERVENTION_ACTION_TEMPLATE]);
  const [institutionIds, setInstitutionIds] = useState([zoneToInstitution(masterCase.zone, masterCase.category)]);
  const [days, setDays] = useState(30);

  const toggleInstitution = (id) => setInstitutionIds((ids) => ids.includes(id) ? ids.filter((x) => x !== id) : [...ids, id]);
  ...
  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-3.5 space-y-3">
      <p className="text-[11px] font-bold text-indigo-800 flex items-center gap-1"><Wrench size={13} /> Nuevo plan de intervención</p>
      <div>
        <label className="text-[11px] font-semibold text-slate-500">Problema</label>
        <textarea value={problem} onChange={(e) => setProblem(e.target.value)} rows={2} className="w-full mt-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs outline-none focus:border-indigo-400" />
      </div>
      <div>
        <label className="text-[11px] font-semibold text-slate-500">Objetivo</label>
        <textarea value={objective} onChange={(e) => setObjective(e.target.value)} rows={2} className="w-full mt-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs outline-none focus:border-indigo-400" />
      </div>
      <div>
        <label className="text-[11px] font-semibold text-slate-500">Acciones</label>
        <div className="space-y-1.5 mt-1">
          {actions.map((a, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <input value={a} onChange={(e) => updateAction(i, e.target.value)} className="flex-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs outline-none focus:border-indigo-400" />
              <button onClick={() => removeAction(i)} className="text-slate-400 hover:text-red-500"><X size={14} /></button>
            </div>
          ))}
          <button onClick={() => setActions((a) => [...a, ""])} className="text-[11px] font-semibold text-indigo-600 flex items-center gap-1"><Plus size={12} /> Agregar acción</button>
        </div>
      </div>
      <div>
        <label className="text-[11px] font-semibold text-slate-500">Instituciones involucradas</label>
        <div className="flex flex-wrap gap-1.5 mt-1">
          {INSTITUTIONS.filter((i) => i.id !== "otra").map((inst) => (
            <button key={inst.id} onClick={() => toggleInstitution(inst.id)} className={`text-[11px] font-semibold rounded-full px-2.5 py-1 border transition ${institutionIds.includes(inst.id) ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white border-slate-200 text-slate-500"}`}>{inst.short}</button>
          ))}
        </div>
      </div>
      <div className="flex items-center gap-2">
        <CalendarClock size={14} className="text-slate-400" />
        <label className="text-[11px] font-semibold text-slate-500">Fecha objetivo:</label>
        <input type="number" min={1} value={days} onChange={(e) => setDays(Number(e.target.value) || 1)} className="w-16 rounded-lg border border-slate-200 px-2 py-1 text-xs outline-none" /><span className="text-[11px] text-slate-500">días</span>
      </div>
      <button
        onClick={() => onCreate({ problem, objective, actions: actions.filter((a) => a.trim()), institutionIds, targetDate: Date.now() + days * 86400000 })}
        disabled={!problem.trim() || !objective.trim() || actions.filter((a) => a.trim()).length === 0}
...
function MasterCaseDetailModal({ masterCase, reports, onClose, onCreatePlan, onToggleAction, onMeasure, onCloseCase, onOpenReport }) {
  if (!masterCase) return null;
  const groupedReports = reports.filter((r) => masterCase.reportIds.includes(r.id));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center">
      <div className="bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-xl max-h-[94vh] overflow-y-auto tr-scroll tr-fade-up">
        <div className="sticky top-0 bg-white/95 backdrop-blur flex items-center justify-between px-4 py-3 border-b border-slate-100 z-10">
          <div>
            <span className="tr-mono text-sm font-bold text-slate-800">{masterCase.code}</span>
            <p className="text-[10px] text-slate-400">Caso maestro — problema recurrente</p>
          </div>
          <button onClick={onClose}><X size={20} className="text-slate-400" /></button>
        </div>
        ...
        <div className="p-4">
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${MASTER_CASE_STATUS_META[masterCase.status].badge}`}>{MASTER_CASE_STATUS_META[masterCase.status].label}</span>
          </div>
          <p className="text-sm font-bold text-slate-800">{getZone(masterCase.zone).name}</p>
          <p className="text-xs text-slate-400 mb-3">{groupedReports.length} incidencias agrupadas · creado {timeAgo(masterCase.createdAt)}</p>

          <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 mb-4">
            <p className="text-[11px] font-bold text-slate-600 mb-2">Incidencias que forman este caso maestro</p>
            <div className="space-y-1.5">
              {groupedReports.map((r) => (
                <button key={r.id} onClick={() => onOpenReport(r.id)} className="w-full flex items-center gap-2 text-left hover:bg-white rounded-lg px-1.5 py-1 transition">
                  <img src={r.photos[0]} className="w-8 h-8 rounded object-cover shrink-0" alt="" />
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-semibold text-slate-700 truncate">{r.code} · {r.desc}</p>
                  </div>
          ...
          {!plan && <InterventionPlanForm masterCase={masterCase} onCreate={(data) => onCreatePlan(masterCase.id, data)} />}

          {plan && (
            <div className="rounded-xl border border-indigo-200 bg-indigo-50/60 p-3.5 space-y-3">
              <p className="text-[11px] font-bold text-indigo-800 flex items-center gap-1"><Wrench size={13} /> Plan de intervención</p>
              <div>
                <p className="text-[11px] font-semibold text-slate-500">Problema</p>
                <p className="text-xs text-slate-700">{plan.problem}</p>
              </div>
              <div>
                <p className="text-[11px] font-semibold text-slate-500">Objetivo</p>
                <p className="text-xs text-slate-700">{plan.objective}</p>
              </div>
              <div>
                <p className="text-[11px] font-semibold text-slate-500 mb-1.5">Acciones ({doneActions}/{plan.actions.length})</p>
                <div className="space-y-1.5">
                  {plan.actions.map((a) => (
                    <button key={a.id} onClick={() => onToggleAction(masterCase.id, a.id)} className="w-full flex items-center gap-2 text-left bg-white rounded-lg border border-slate-200 px-2.5 py-1.5 hover:border-indigo-300">
                      {a.done ? <CheckSquare size={15} className="text-emerald-500 shrink-0" /> : <Square size={15} className="text-slate-300 shrink-0" />}
                      <span className={`text-xs font-medium ${a.done ? "text-slate-400 line-through" : "text-slate-700"}`}>{a.label}</span>
                    </button>
                  ))}
                </div>
              </div>
              <div className="flex items-center gap-1.5 flex-wrap">
                <p className="text-[11px] font-semibold text-slate-500">Instituciones:</p>
                {plan.institutionIds.map((id) => <span key={id} className="text-[10px] font-semibold bg-white border border-indigo-200 text-indigo-700 rounded-full px-2 py-0.5">{getInstitution(id).short}</span>)}
              </div>
              <p className={`text-[11px] font-semibold flex items-center gap-1 ${overdueTarget ? "text-red-600" : "text-slate-500"}`}><CalendarClock size={12} /> Fecha objetivo: {new Date(plan.targetDate).toLocaleDateString("es-DO")} {overdueTarget && "· vencida"}</p>

              {plan.measuredAt ? (
                <div className={`rounded-lg p-2.5 flex items-center gap-2 ${plan.reduced ? "bg-emerald-50 border border-emerald-200" : "bg-amber-50 border border-amber-200"}`}>
                  {plan.reduced ? <ArrowDownRight size={16} className="text-emerald-600" /> : <ArrowUpRight size={16} className="text-amber-600" />}
                  <p className={`text-xs font-semibold ${plan.reduced ? "text-emerald-700" : "text-amber-700"}`}>
                    {plan.resultCount} incidencias nuevas desde el plan (línea base: {plan.baselineCount}) — {plan.reduced ? "el problema disminuyó" : "sin mejora medible aún"}
                  </p>
                </div>
              ) : (
                <button onClick={() => onMeasure(masterCase.id)} className="w-full rounded-xl border border-indigo-300 text-indigo-700 text-xs font-bold py-2 hover:bg-indigo-100">¿Disminuyeron las incidencias? Medir resultado</button>
              )}

              {masterCase.status !== "cerrado" && (
                <button onClick={() => onCloseCase(masterCase.id)} disabled={!allActionsDone} className="w-full rounded-xl bg-slate-800 text-white text-xs font-bold py-2 disabled:opacity-30">
                  {allActionsDone ? "Cerrar caso maestro" : `Completa las ${plan.actions.length - doneActions} acciones restantes para cerrar`}
                </button>
              )}
            </div>
          )}
...
function AdminDashboard({ reports }) {
  const total = reports.length;
  const resolved = reports.filter((r) => ["resuelto", "cerrado"].includes(r.status)).length;
  ...
  const byStatus = STATUS_ORDER.map((s) => ({ name: STATUS_META[s].label, value: reports.filter((r) => r.status === s).length })).filter((d) => d.value > 0);
  const byInstitution = INSTITUTIONS.filter((i) => i.id !== "otra").map((inst) => ({
    name: inst.short,
    reportes: reports.filter((r) => r.institutionId === inst.id).length,
    resueltos: reports.filter((r) => r.institutionId === inst.id && ["resuelto", "cerrado"].includes(r.status)).length,
  ...
  const byPriority = PRIORITY_ORDER.map((p) => ({ name: PRIORITY_META[p].label, value: reports.filter((r) => r.priority === p).length }));
  ...
  const criticalActive = reports.filter((r) => r.priority === "critica" && isActiveReport(r)).sort((a, b) => slaRemaining(a) - slaRemaining(b)).slice(0, 5);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pt-4 pb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        <KpiCard label="Incidencias totales" value={total} Icon={FileText} tone="blue" sub="Últimos 26 días" />
        <KpiCard label="Tasa de resolución" value={`${Math.round((resolved / total) * 100)}%`} Icon={CheckCircle2} tone="teal" sub={`${resolved} de ${total} incidencias`} />
        <KpiCard label="Escaladas" value={escalated} Icon={TrendingUp} tone="amber" sub="Requieren arbitraje de supervisor" />
      ...
      <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2.5 mt-2">Dashboard ejecutivo — KPIs de gestión</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <KpiCard label="First Response Time" value={formatKpiHours(kpis.frtHours)} Icon={Clock} tone="blue" sub={kpis.frtSample ? `Promedio sobre ${kpis.frtSample} casos` : "Sin datos aún"} />
        <KpiCard label="Average Resolution Time" value={formatKpiHours(kpis.artHours)} Icon={CheckCircle2} tone="teal" sub={kpis.artSample ? `Promedio sobre ${kpis.artSample} casos cerrados` : "Sin casos cerrados"} />
        <KpiCard label="SLA Compliance" value={kpis.slaCompliancePct == null ? "—" : `${Math.round(kpis.slaCompliancePct)}%`} Icon={Gauge} tone={kpis.slaCompliancePct != null && kpis.slaCompliancePct < 80 ? "red" : "teal"} sub={kpis.slaSample ? `${kpis.slaSample} expedientes con SLA` : "Ningún expediente con SLA aún"} />
        <KpiCard label="Backlog" value={kpis.backlog} Icon={ClipboardList} tone="amber" sub="Expedientes activos ahora mismo" />
        <KpiCard label="Aging promedio" value={formatKpiHours(kpis.agingHours)} Icon={TimerReset} tone="amber" sub="Antigüedad de los casos activos" />
        <KpiCard label="Reopen Rate" value={kpis.reopenRatePct == null ? "—" : `${Math.round(kpis.reopenRatePct)}%`} Icon={RefreshCw} tone={kpis.reopenRatePct > 15 ? "red" : "blue"} sub="Sobre expedientes cerrados alguna vez" />
        <KpiCard label="Escalation Rate" value={`${Math.round(kpis.escalationRatePct)}%`} Icon={TrendingUp} tone={kpis.escalationRatePct > 20 ? "red" : "blue"} sub="Expedientes escalados sobre el total" />
        <KpiCard label="Resolution Rate" value={`${Math.round(kpis.resolutionRatePct)}%`} Icon={BadgeCheck} tone="teal" sub="Resuelto o cerrado sobre el total" />
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-4">
          <p className="text-sm font-bold text-slate-700 mb-3">Incidencias por prioridad</p>
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={byPriority}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                ...
                <Tooltip wrapperStyle={{ fontSize: 12 }} />
                <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                  {byPriority.map((d, i) => <Cell key={i} fill={PRIORITY_META[PRIORITY_ORDER[i]].color} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-2xl bg-white border border-red-100 p-4">
          <p className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1.5"><AlertTriangle size={14} className="text-red-500" /> Incidencias críticas activas</p>
          <p className="text-[11px] text-slate-400 mb-3">Casos de prioridad crítica que aún no se han resuelto, ordenados por tiempo restante.</p>
          <div className="space-y-2">
            {criticalActive.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center gap-2.5 rounded-xl border border-slate-100 p-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[9px] tr-mono text-slate-400">{r.code}</p>
                    <p className="text-xs font-semibold text-slate-700 truncate">{r.desc}</p>
            ...
            {criticalActive.length === 0 && <p className="text-xs text-slate-400 text-center py-6">No hay incidencias críticas pendientes ahora mismo.</p>}
          </div>
        </div>
...
function AuditLogView({ reports, masterCases, institutionScope, onOpenReport, onOpenMasterCase }) {
  const [q, setQ] = useState("");
  const [userFilter, setUserFilter] = useState("todos");
  const [campoFilter, setCampoFilter] = useState("Todos");
  const [range, setRange] = useState("todo");

  const allEntries = useMemo(() => buildAuditLog(reports, masterCases), [reports, masterCases]);
  const scoped = useMemo(
    () => (institutionScope ? allEntries.filter((e) => e.kind === "masterCase" || e.institutionId === institutionScope) : allEntries),
    [allEntries, institutionScope]
  );
  const users = useMemo(() => Array.from(new Set(scoped.map((e) => e.by))).sort(), [scoped]);
  ...
  const filtered = useMemo(() => {
    ...
    return scoped.filter((e) => {
      ...
      if (query) {
        const hay = `${e.caseCode} ${e.by} ${e.action} ${e.motivo || ""} ${e.anterior} ${e.nuevo}`.toLowerCase();
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pt-4 pb-8">
      <SectionHeading
        ...
        action={
          <button
            onClick={() => exportAuditCsv(filtered, `auditoria-tureporte-${new Date().toISOString().slice(0, 10)}.csv`)}
      ...
      <p className="text-xs text-slate-400 -mt-3 mb-4">
        {filtered.length} de {scoped.length} registros · bitácora de solo lectura, ningún rol puede eliminar auditoría.
      </p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-3.5 mb-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        ...
        <select value={userFilter} onChange={(e) => setUserFilter(e.target.value)} className="rounded-lg border border-slate-200 px-2.5 py-2 text-xs bg-white outline-none focus:ring-2 focus:ring-blue-400">
          <option value="todos">Todos los usuarios</option>
          {users.map((u) => <option key={u} value={u}>{u}</option>)}
        </select>
        <select value={campoFilter} onChange={(e) => setCampoFilter(e.target.value)} className="rounded-lg border border-slate-200 px-2.5 py-2 text-xs bg-white outline-none focus:ring-2 focus:ring-blue-400">
          {AUDIT_CAMPO_OPTIONS.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={range} onChange={(e) => setRange(e.target.value)} className="rounded-lg border border-slate-200 px-2.5 py-2 text-xs bg-white outline-none focus:ring-2 focus:ring-blue-400">
          {AUDIT_RANGE_OPTIONS.map((r) => <option key={r.id} value={r.id}>{r.label}</option>)}
        </select>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
        <div className="hidden md:grid grid-cols-[110px_120px_90px_1fr_170px_90px] gap-2 px-4 py-2.5 bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-wide">
          <span>Fecha</span><span>Usuario</span><span>Caso</span><span>Acción</span><span>Cambio</span><span>Motivo</span>
        </div>
        <div className="max-h-[65vh] overflow-y-auto tr-scroll divide-y divide-slate-100">
          {filtered.map((e) => (
            <button
              key={e.id}
              onClick={() => (e.kind === "masterCase" ? onOpenMasterCase?.(e.caseId) : onOpenReport?.(e.caseId))}
            ...
            >
              <span className="text-[10px] tr-mono text-slate-400 col-span-2 md:col-span-1">{formatDateTime(e.at)}</span>
              <span className="text-xs font-semibold text-slate-700 truncate">{e.by}</span>
              <span className="text-[10px] tr-mono text-blue-700 truncate">{e.caseCode}</span>
              <span className="text-xs text-slate-600 truncate">{e.action}</span>
              <span className="text-[11px] text-slate-500 truncate">
                {e.campo === "Estado" ? <><span className="text-slate-400">{e.anterior}</span> → <span className="font-semibold text-slate-700">{e.nuevo}</span></> : <span className="text-slate-400">{e.campo}</span>}
              </span>
              <span className="text-[11px] text-slate-400 truncate" title={e.motivo || ""}>{e.motivo || "—"}</span>
            </button>
          ))}
          {filtered.length === 0 && <p className="text-xs text-slate-400 text-center py-10">No hay registros de auditoría con estos filtros.</p>}
        </div>
      </div>
    </div>
  );
...
function RuleToggle({ on, onClick }) {
  return (
    <button onClick={onClick} className={`w-9 h-5 rounded-full transition relative shrink-0 ${on ? "bg-teal-500" : "bg-slate-200"}`}>
      <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition ${on ? "left-4.5" : "left-0.5"}`} style={{ left: on ? "18px" : "2px" }} />
    </button>
  );
}
...
function AdminRulesView({ rules, setRules }) {
  ...
  const addRoutingRule = () => {
    ...
    const rule = { id: uid("rule-rt"), enabled: true, categoryId: newRoutingCategory, subcategoryId: newRoutingSubcategory || null, institutionId: newRoutingInstitution };
    setRules((r) => ({ ...r, routing: [...r.routing, rule] }));
    setNewRoutingCategory(""); setNewRoutingSubcategory(""); setNewRoutingInstitution("");
  };
  const toggleRoutingRule = (id) => setRules((r) => ({ ...r, routing: r.routing.map((x) => (x.id === id ? { ...x, enabled: !x.enabled } : x)) }));
  const removeRoutingRule = (id) => setRules((r) => ({ ...r, routing: r.routing.filter((x) => x.id !== id) }));

  const addSlaRule = () => {
    const hours = Number(newSlaHours);
    if (!newSlaPriority || !hours || hours <= 0) return;
    const rule = { id: uid("rule-sla"), enabled: true, priority: newSlaPriority, slaHours: hours };
    setRules((r) => ({ ...r, sla: [...r.sla, rule] }));
  };
  const toggleSlaRule = (id) => setRules((r) => ({ ...r, sla: r.sla.map((x) => (x.id === id ? { ...x, enabled: !x.enabled } : x)) }));
  const removeSlaRule = (id) => setRules((r) => ({ ...r, sla: r.sla.filter((x) => x.id !== id) }));

  const totalActive = rules.routing.filter((r) => r.enabled).length + rules.sla.filter((r) => r.enabled).length + (rules.notifySlaBreach ? 1 : 0) + 1;

  return (
    <div className="max-w-4xl mx-auto px-4 pt-4 pb-10">
      <SectionHeading eyebrow="Automatización y enrutamiento" title="Motor de reglas" />
      <p className="text-xs text-slate-400 -mt-3 mb-5">
        {totalActive} reglas activas · se aplican automáticamente al crear un reporte y al clasificar/asignar una incidencia. Mientras no agregues una regla, el sistema usa su comportamiento por defecto.
      </p>

      {/* ------- Enrutamiento ------- */}
      <div className="rounded-2xl border border-slate-200 bg-white p-4 mb-5">
        <p className="text-sm font-bold text-slate-700 mb-1">Reglas de enrutamiento</p>
        <p className="text-[11px] text-slate-400 mb-3">IF categoría [+ subcategoría] THEN institución responsable. La primera regla habilitada que coincida gana; si ninguna aplica, se usa el enrutamiento automático por zona.</p>

        <div className="space-y-2 mb-3">
          {rules.routing.length === 0 && <p className="text-xs text-slate-400 py-2">Aún no hay reglas de enrutamiento — se usa el enrutamiento automático por zona/categoría.</p>}
          {rules.routing.map((rule) => (
            <div key={rule.id} className={`flex items-center gap-2.5 rounded-xl border px-3 py-2 ${rule.enabled ? "border-slate-200" : "border-slate-100 opacity-50"}`}>
              <RuleToggle on={rule.enabled} onClick={() => toggleRoutingRule(rule.id)} />
              <span className="text-xs text-slate-600 flex-1 min-w-0 truncate">
                <span className="font-semibold text-slate-700">IF</span> {getCategory(rule.categoryId).label}{rule.subcategoryId ? ` → ${getSubcategory(rule.subcategoryId)?.label}` : ""}
                <span className="font-semibold text-slate-700"> THEN</span> {getInstitution(rule.institutionId).short}
              </span>
              <button onClick={() => removeRoutingRule(rule.id)} className="shrink-0 text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
            </div>
          ))}
        </div>

        <div className="grid sm:grid-cols-[1fr_1fr_1fr_auto] gap-2">
          <select value={newRoutingCategory} onChange={(e) => { setNewRoutingCategory(e.target.value); setNewRoutingSubcategory(""); }} className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
            <option value="">Categoría…</option>
            {CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
          </select>
          <select value={newRoutingSubcategory} onChange={(e) => setNewRoutingSubcategory(e.target.value)} disabled={!newRoutingCategory} className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white disabled:bg-slate-50">
            <option value="">Cualquier subcategoría</option>
            {getSubcategoriesFor(newRoutingCategory).map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}
          </select>
          <select value={newRoutingInstitution} onChange={(e) => setNewRoutingInstitution(e.target.value)} className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
            <option value="">Institución…</option>
            {INSTITUTIONS.map((i) => <option key={i.id} value={i.id}>{i.short}</option>)}
          </select>
          <button onClick={addRoutingRule} disabled={!newRoutingCategory || !newRoutingInstitution} className="flex items-center justify-center gap-1 rounded-lg bg-blue-900 text-white text-xs font-semibold px-3 py-1.5 disabled:opacity-40">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 mb-5">
        ...
        <p className="text-[11px] text-slate-400 mb-3">
          IF prioridad THEN horas de SLA. Por defecto: {PRIORITY_ORDER.map((p) => `${PRIORITY_META[p].label} ${PRIORITY_META[p].slaHours}h`).join(" · ")}.
        </p>

        <div className="space-y-2 mb-3">
          {rules.sla.length === 0 && <p className="text-xs text-slate-400 py-2">Aún no hay reglas de SLA — se usan los valores por defecto de cada prioridad.</p>}
          {rules.sla.map((rule) => (
            <div key={rule.id} className={`flex items-center gap-2.5 rounded-xl border px-3 py-2 ${rule.enabled ? "border-slate-200" : "border-slate-100 opacity-50"}`}>
              <RuleToggle on={rule.enabled} onClick={() => toggleSlaRule(rule.id)} />
              <span className="text-xs text-slate-600 flex-1 min-w-0 truncate">
                <span className="font-semibold text-slate-700">IF</span> prioridad = {PRIORITY_META[rule.priority].label}
                <span className="font-semibold text-slate-700"> THEN</span> SLA = {rule.slaHours}h
              </span>
              <button onClick={() => removeSlaRule(rule.id)} className="shrink-0 text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
            </div>
          ))}
        </div>

        <div className="grid sm:grid-cols-[1fr_1fr_auto] gap-2">
          <select value={newSlaPriority} onChange={(e) => setNewSlaPriority(e.target.value)} className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs bg-white">
            {PRIORITY_ORDER.map((p) => <option key={p} value={p}>{PRIORITY_META[p].label}</option>)}
          </select>
          <input type="number" min="1" value={newSlaHours} onChange={(e) => setNewSlaHours(e.target.value)} placeholder="Horas" className="rounded-lg border border-slate-200 px-2 py-1.5 text-xs" />
          <button onClick={addSlaRule} className="flex items-center justify-center gap-1 rounded-lg bg-blue-900 text-white text-xs font-semibold px-3 py-1.5">
            <Plus size={13} /> Agregar
          </button>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <p className="text-sm font-bold text-slate-700 mb-1">Alertas de SLA</p>
          <p className="text-[11px] text-slate-400 mb-3">IF SLA vencido THEN notificar al supervisor.</p>
          <div className="flex items-center gap-2.5">
            <RuleToggle on={rules.notifySlaBreach} onClick={() => setRules((r) => ({ ...r, notifySlaBreach: !r.notifySlaBreach }))} />
            <span className="text-xs text-slate-600">{rules.notifySlaBreach ? "Activado — los vencimientos aparecen en Avisos de gestores/supervisores" : "Desactivado — los vencimientos no generan notificación"}</span>
          </div>
        </div>

        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <p className="text-sm font-bold text-slate-700 mb-1">Problemas recurrentes</p>
          <p className="text-[11px] text-slate-400 mb-3">IF múltiples casos en misma zona/categoría THEN sugerir problema recurrente.</p>
          <div className="flex items-center gap-3">
            <label className="text-[11px] text-slate-500 flex items-center gap-1.5">
              Mínimo
              <input type="number" min="2" value={rules.recurringMinCount} onChange={(e) => setRules((r) => ({ ...r, recurringMinCount: Number(e.target.value) || r.recurringMinCount }))} className="w-14 rounded-lg border border-slate-200 px-2 py-1 text-xs" />
              casos
            </label>
            <label className="text-[11px] text-slate-500 flex items-center gap-1.5">
              en
              <input type="number" min="1" value={rules.recurringWindowDays} onChange={(e) => setRules((r) => ({ ...r, recurringWindowDays: Number(e.target.value) || r.recurringWindowDays }))} className="w-14 rounded-lg border border-slate-200 px-2 py-1 text-xs" />
              días
            </label>
          </div>
        </div>
...
export default function App() {
  ...
  const notifications = useMemo(() => buildNotificationsForUser(session, reports, rules), [session, reports, rules]);

  const handleLogin = (user) => {
    setSession(user);
    setView(user.role === "ciudadano" ? "feed" : user.role === "supervisor" ? "supervisorCenter" : "dashboard");
  };
  ...
  const handlePublish = ({ category, subcategory, desc, zone, anonymous, photos }) => {
    ...
    const routedInstitutionId = applyRoutingRule(rules, category, subcategory, zoneToInstitution(zone, category));
    const slaHours = applySlaHours(rules, suggestedPriority, PRIORITY_META[suggestedPriority].slaHours);
    const newReport = {
      id: uid("r"),
      code: `INC-${new Date(now).getFullYear()}-${num}`,
      desc, category, zone, subcategory, isAnonymous: anonymous,
      status: "nuevo", priority: suggestedPriority,
      authorId: session.id, photos, evidenceBefore: photos, evidenceDuring: [], evidenceAfter: [],
      createdAt: now, commentsCount: 0,
      institutionId: routedInstitutionId,
      department: null, responsable: null,
      slaHours, dueAt: now + slaHours * 3600000,
      isDuplicate: false, duplicateOf: null, satisfaction: null, reopenedCount: 0,
      distanceM: 40,
      tasks: [], inspections: [],
      timeline: [{ at: now, label: "Reporte creado por el ciudadano", by: session.isAnonymous ? "Usuario anónimo" : session.name, toStatus: "nuevo" }],
    };
  ...
  const handleReportAction = (reportId, action, payload = {}) => {
    setReports((rs) => rs.map((r) => {
      ...
      const entry = { at: Date.now(), by: session.name, toStatus: action.to, reason: payload.reason || null };
      ...
      if (action.needsClassification) {
        ...
        updated.institutionId = payload.institutionId || applyRoutingRule(rules, r.category, r.subcategory, r.institutionId);
        updated.department = payload.department || null;
        updated.responsable = payload.responsable || null;
        updated.dueAt = Date.now() + applySlaHours(rules, updated.priority, PRIORITY_META[updated.priority]?.slaHours || r.slaHours) * 3600000;
  ...
  const handleAddEvidence = (reportId, url, phase) => {
    setReports((rs) => rs.map((r) => {
      if (r.id !== reportId) return r;
      const key = phase === "after" ? "evidenceAfter" : phase === "during" ? "evidenceDuring" : "evidenceBefore";
      const updated = { ...r, [key]: [...(r[key] || []), url] };
      updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: `Evidencia de ${phase === "after" ? "resolución" : phase === "during" ? "gestión" : "reporte"} agregada` }];
      return updated;
    }));
  ...
  const handleAddTask = (reportId, taskData) => {
    setReports((rs) => rs.map((r) => {
      ...
      const blocking = dependsOn ? (r.tasks || []).find((t) => t.id === dependsOn) : null;
      const status = blocking && blocking.status !== "completada" ? "bloqueada" : "pendiente";
      const task = {
        id: uid("tk"), reportId, title: taskData.title, description: taskData.description || "",
        assignee: taskData.assignee || null, priority: taskData.priority || r.priority,
        status, dependsOn, dueAt: taskData.dueAt || null,
        createdAt: Date.now(), completedAt: null, comment: "",
      };
      const updated = { ...r, tasks: [...(r.tasks || []), task] };
      updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: `Tarea creada: "${task.title}"${task.assignee ? ` · Responsable: ${task.assignee}` : ""}` }];
      return updated;
    }));
    ...
    setReports((rs) => rs.map((r) => {
      ...
      const tasks = (r.tasks || []).map((t) => {
        ...
        if (t.dependsOn) {
          ...
          if (dep && dep.status !== "completada" && newStatus !== "bloqueada") return t; // impedido
        }
        return { ...t, status: newStatus, completedAt: newStatus === "completada" ? Date.now() : t.completedAt };
      });
      const unblocked = tasks.map((t) => {
        if (t.status !== "bloqueada" || !t.dependsOn) return t;
        const dep = tasks.find((x) => x.id === t.dependsOn);
        return dep && dep.status === "completada" ? { ...t, status: "pendiente" } : t;
      });
      const changedTask = unblocked.find((t) => t.id === taskId);
      const updated = { ...r, tasks: unblocked };
      if (changedTask) {
        updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: `Tarea "${changedTask.title}" → ${TASK_STATUS_META[changedTask.status].label}` }];
      }
      return updated;
    }));
  ...
  const handleRequestInspection = (reportId, data) => {
    setReports((rs) => rs.map((r) => {
      if (r.id !== reportId) return r;
      const inspection = {
        id: uid("insp"), reportId, requestedBy: session.name, requestedAt: Date.now(),
        assignee: data.assignee || r.responsable || null, scheduledFor: data.scheduledFor || null,
        status: "solicitada", result: null, observations: "", photos: [],
      };
      const updated = { ...r, inspections: [...(r.inspections || []), inspection] };
      updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: `Inspección solicitada${inspection.assignee ? ` · Responsable: ${inspection.assignee}` : ""}` }];
      return updated;
    }));
  ...
  const handleRecordInspectionResult = (reportId, inspectionId, data) => {
    setReports((rs) => rs.map((r) => {
      if (r.id !== reportId) return r;
      const inspections = (r.inspections || []).map((insp) => insp.id === inspectionId
        ? { ...insp, status: "realizada", result: data.result, observations: data.observations || "", performedAt: Date.now(), photos: data.photoUrl ? [...(insp.photos || []), data.photoUrl] : insp.photos }
        : insp);
      const updated = { ...r, inspections };
      updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: `Resultado de inspección registrado: ${INSPECTION_RESULT_META[data.result]?.label || data.result}` }];
      return updated;
    }));
  ...
  const handleMergeDuplicate = (reportId, targetId) => {
    setReports((rs) => rs.map((r) => {
      if (r.id !== reportId) return r;
      const updated = { ...r, status: "duplicado", duplicateOf: targetId, isDuplicate: true };
      updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, toStatus: "duplicado", label: `Fusionado con el reporte ${rs.find((x) => x.id === targetId)?.code || targetId}`, reason: "Mismo problema, misma zona y fecha cercana." }];
      return updated;
    }));
  ...
  const handleConfirmResolution = (reportId, solved) => {
    if (solved) {
      setReports((rs) => rs.map((r) => {
        ...
        updated.timeline = [...r.timeline, { at: Date.now(), by: session.name, label: "El ciudadano confirmó que el problema fue solucionado" }];
        return updated;
      }));
      showToast("Gracias por confirmar la resolución");
    } else {
      handleReportAction(reportId, TRANSITIONS.cerrado[0], { reason: "El ciudadano indica que el problema continúa presente." });
    }
  };
  ...
  const handleCreateMasterCase = (category, zone, reportIds) => {
    ...
    const newCase = {
      ...
      category, zone, reportIds,
      status: "abierto",
      createdAt: now, createdBy: session.name,
      interventionPlan: null,
      timeline: [{ at: now, by: session.name, label: `Caso maestro creado agrupando ${reportIds.length} incidencias` }],
    };
  ...
  const handleCreateInterventionPlan = (masterCaseId, planData) => {
    setMasterCases((list) => list.map((mc) => {
      if (mc.id !== masterCaseId) return mc;
      const plan = {
        problem: planData.problem, objective: planData.objective,
        actions: planData.actions.map((label) => ({ id: uid("act"), label, done: false })),
        institutionIds: planData.institutionIds,
        targetDate: planData.targetDate,
        createdAt: Date.now(),
        baselineCount: mc.reportIds.length,
        measuredAt: null, resultCount: null, reduced: null,
      };
      const updated = { ...mc, status: "en_intervencion", interventionPlan: plan };
      updated.timeline = [...mc.timeline, { at: Date.now(), by: session.name, label: "Plan de intervención creado" }];
      return updated;
    }));
  ...
  const handleToggleInterventionAction = (masterCaseId, actionId) => {
    setMasterCases((list) => list.map((mc) => {
      ...
      const actions = mc.interventionPlan.actions.map((a) => a.id === actionId ? { ...a, done: !a.done } : a);
  ...
  const handleMeasureIntervention = (masterCaseId) => {
    setMasterCases((list) => list.map((mc) => {
      ...
      const newIncidents = reports.filter((r) => r.category === mc.category && r.zone === mc.zone && r.createdAt > since).length;
      const reduced = newIncidents < mc.interventionPlan.baselineCount;
      const updated = { ...mc, interventionPlan: { ...mc.interventionPlan, measuredAt: Date.now(), resultCount: newIncidents, reduced } };
      updated.timeline = [...mc.timeline, { at: Date.now(), by: session.name, label: `Resultado medido: ${newIncidents} incidencias nuevas (línea base: ${mc.interventionPlan.baselineCount}) — ${reduced ? "mejoró" : "sin mejora"}` }];
      return updated;
    }));
  ...
  const handleCloseMasterCase = (masterCaseId) => {
    setMasterCases((list) => list.map((mc) => {
      ...
      updated.timeline = [...mc.timeline, { at: Date.now(), by: session.name, label: "Caso maestro cerrado" }];
      return updated;
    }));
  ...
  const detailMasterCase = masterCases.find((mc) => mc.id === detailMasterCaseId) || null;

  return (
    <div className="tr-root h-full">
      <GlobalStyle />
      <AppShell user={session} view={view} setView={setView} onLogout={handleLogout} unreadCount={unreadCount}>
        ...
        {session.role === "ciudadano" && view === "notify" && <NotificationsView notifications={notifications} onOpenReport={openReportById} />}
        ...
        {session.role === "supervisor" && view === "supervisorCenter" && (
          <SupervisorControlCenter reports={reports} user={session} onOpenDetail={setDetailReport} masterCases={masterCases} onCreateMasterCase={(cat, zone, ids) => { const mcId = handleCreateMasterCase(cat, zone, ids); openMasterCaseById(mcId); }} onOpenMasterCase={openMasterCaseById} />
        )}
        {(session.role === "gestor" || session.role === "supervisor") && view === "dashboard" && <GestorDashboard reports={reports} user={session} onOpenDetail={setDetailReport} showToast={showToast} />}
        {(session.role === "gestor" || session.role === "supervisor" || session.role === "admin") && view === "masterCases" && <MasterCasesView masterCases={masterCases} onOpen={openMasterCaseById} />}
        {(session.role === "gestor" || session.role === "supervisor") && view === "stats" && <GestorStats reports={reports} user={session} />}
        {(session.role === "gestor" || session.role === "supervisor") && view === "map" && (
          <OperationalMap reports={reports} institutionScope={session.institutionId} mapMode={mapMode} setMapMode={setMapMode} onOpenReport={openReportById} title="Mapa de incidencias" eyebrow="Cobertura territorial" />
        )}
        {session.role === "supervisor" && view === "audit" && (
          <AuditLogView reports={reports} masterCases={masterCases} institutionScope={session.institutionId} onOpenReport={openReportById} onOpenMasterCase={openMasterCaseById} />
        )}
        {(session.role === "gestor" || session.role === "supervisor") && view === "notify" && <NotificationsView notifications={notifications} onOpenReport={openReportById} />}
        {(session.role === "gestor" || session.role === "supervisor") && view === "profile" && (
        ...
        {session.role === "admin" && view === "map" && (
          <OperationalMap reports={reports} mapMode={mapMode} setMapMode={setMapMode} onOpenReport={openReportById} title="Mapa de reportes" eyebrow="Control nacional" />
        )}
        {session.role === "admin" && view === "audit" && (
          <AuditLogView reports={reports} masterCases={masterCases} institutionScope={null} onOpenReport={openReportById} onOpenMasterCase={openMasterCaseById} />
        )}
        {session.role === "admin" && view === "rules" && <AdminRulesView rules={rules} setRules={setRules} />}
        {session.role === "admin" && view === "users" && <AdminUsers users={users} onCreate={handleCreateUser} />}
        {session.role === "admin" && view === "notify" && <NotificationsView notifications={notifications} onOpenReport={openReportById} />}
      ...
      <MasterCaseDetailModal
        ...
        onOpenReport={(id) => { setDetailMasterCaseId(null); openReportById(id); }}
      />
      <Toast toast={toast} />
    </div>
  );
